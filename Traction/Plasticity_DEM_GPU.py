...
# Cohesion calibration mode for DP.
...
# Abaqus deck uses TYPE=COMPRESSION, so the default matches σ_c.
COHESION_CALIBRATION = os.environ.get('DP_COHESION_MODE', 'compression').lower()

SIGMA_TENSION_TARGET = os.environ.get('DP_SIGMA_TENSION_TARGET', None)

if COHESION_CALIBRATION == 'compression':
...
if COHESION_CALIBRATION == 'compression':
    ...
    logger.info(f"  Implied uniaxial tension yield for this linear DP form: σ_t = {sigma_t_implied:.4f} MPa")
elif COHESION_CALIBRATION == 'tension':
    sigma_t_implied = cohesion_d.item() / (1.0 + float(TAN_BETA.item()) / 3.0)
