...
- `DP_SEED`: global random seed used by Python, NumPy, and PyTorch. Defaults to `2022`.
- `DP_RESULTS_ROOT`: output root used by the multiseed batch runner.
- `DP_SEEDS`: comma-separated seed list for the batch runner, default `2022,2023,2024,2025,2026`.
- `DP_STUDIES`: optional comma-separated study filter for the batch runner.
...
### Multiseed sweeps
The `run_multiseed_sweeps.sh` wrapper runs the ablation, learning-rate, and network-width sweeps across a seed list and writes one summary CSV per study under `multiseed_results/`.

Example:
```bash
bash run_multiseed_sweeps.sh
```

You can limit the studies or override seeds:
```bash
DP_STUDIES=ablation DP_SEEDS=2022,2023,2024,2025,2026 bash run_multiseed_sweeps.sh
```
...
- `multiseed_results/<study>/summary.csv`: per-seed rows plus aggregate mean/std rows from the batch runner.
