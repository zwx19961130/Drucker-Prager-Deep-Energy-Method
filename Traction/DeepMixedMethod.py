...
class DruckerPragerPlasticity(torch.autograd.Function):
            ...
            if True:
                ...
                def _compute_update(gamma_vals):
                    ...
                    if psi_small:
                        ...
                        p_smooth = p_trial_p + K * gamma_vals * TAN_PSI
                    ...
                    # stress, eps_p, and PEEQ remain mutually consistent for energy output.
                    ...
                    # Exact apex stress has q=0, but dp_invariants adds eps_global
                    # under sqrt and reports a numerical q floor. Do not let that
                    # artificial 1e-6 residual drive Newton/apex flip diagnostics.
    ...
    def backward(ctx, grad_stress, grad_eps_p, grad_peeq):
        ...
        if plastic_mask.any() and DP_TANGENT_MODE != 'elastic':
            ...
            if s_trial_p is not None:
                ...
                if apex_mask.any():
                    ...
                    if num_apex_points > 0:
                         ...
                         if isinstance(Denom_CAT_apex_safe, torch.Tensor):
                             dgamma_dstrain_apex = (K * TAN_BETA / Denom_CAT_apex_safe).reshape(-1, 1, 1) * I_tensor
                         else:
                             dgamma_dstrain_apex = (K * TAN_BETA / Denom_CAT_apex_safe) * I_tensor
                ...
                if grad_eps_p_batched is not None:
                    grad_eps_p_plastic = grad_eps_p_batched[plastic_mask]
                    if grad_eps_p_plastic.numel() > 0:
                        # Use a distinct batch index; using "n" for both batch and tensor indices
                        # makes PyTorch try to broadcast the plastic-point count against 3.
                        Dinv_Calg = torch.einsum('abmn,pmnij->pabij', D_inv_tensor, D_ep_p)
                        J_eps_p = I_sym.unsqueeze(0) - Dinv_Calg
                        grad_eps_p_term_batched[plastic_mask] = torch.einsum('pab,pabij->pij', grad_eps_p_plastic, J_eps_p)
                if grad_peeq_batched is not None:
                    grad_peeq_plastic = grad_peeq_batched[plastic_mask]
                    if grad_peeq_plastic.numel() > 0:
                        grad_peeq_term_batched[plastic_mask] = grad_peeq_plastic.reshape(-1, 1, 1) * peeq_grad_p
...
class DeepMixedMethod:
    ...
    def _get_top_surface_node_mask(self, nodes: Optional[torch.Tensor] = None):
        ...
        if nodes is None:
            return None
        y_top = torch.max(nodes[:, 1])
        tol = max(1e-6, 1e-8 * max(abs(float(y_top.item())), 1.0))
        return torch.isclose(nodes[:, 1], y_top, atol=tol)
    ...
    # Prepare checkpoint directory for robust weight transfer between steps
        ...
        for step in range(1, self.step_max+1):
            ...
            history_plastic_tol = float(getattr(self, 'history_plastic_gate_tol', 1e-10))
            history_plastic = max_peeq_initial > history_plastic_tol
            distributed_plastic = (rho_plast > 1e-2) or (p95_peeq > 1e-5)
            plast_gate = history_plastic or distributed_plastic
            ...
            if self._should_log('step', step):
                ...
                logger.info(
                    ...
                    f"phase={phase}, assembled_weight={target_assembled_weight:.3e}"
                )
            ...
            for adam_epoch in range(adam_epochs):
                ...
                if adam_epoch + 1 == 2000 and not hasattr(self, '_plastic_corrector_applied'):
                    ...
                    with torch.no_grad():
                        ...
                        INCIPIENT_PLASTICITY_WARN_TOL = 1e-5
                        if peeq_max_before < 1e-10 and peeq_max_detected > 1e-10:
                            # FIRST PLASTIC DETECTION: keep previous-step history fixed,
                            # but activate plastic-step equilibrium weights immediately.
                            old_asm_weight = float(getattr(self, '_assembled_equilibrium_active_weight', self.assembled_equilibrium_weight_elastic))
                            self._assembled_equilibrium_active_weight = float(self.assembled_equilibrium_weight_plastic)
                            if not getattr(self, 'disable_collocation', False):
                                self._equilibrium_penalty_on = True
                                self._equilibrium_weight = float(self.equilibrium_penalty_weight_plastic)
                            ...
                            msg = (
                                ...
                                f"Kept previous-step history fixed; switched plastic equilibrium weights for remaining epochs. "
                            ...
                            if peeq_max_detected >= INCIPIENT_PLASTICITY_WARN_TOL:
                                logger.warning(msg)
                            else:
                                logger.info(
                                    msg + f" Below warning threshold ({INCIPIENT_PLASTICITY_WARN_TOL:.1e}); weight switched without escalation."
                                )
                        else:
            ...
            if step_apex is not None:
                ...
                apex_ratio = 100.0 * apex_points_total / plastic_points_total if plastic_points_total > 0 else 0.0
    ...
    def _build_mesh_cache(self, domain):
        ...
        bottom_mask_exact = nodes[:, 1] <= (torch.min(nodes[:, 1]) + tol_y)
        left_mask_exact = nodes[:, 0] <= (torch.min(nodes[:, 0]) + tol_x)
        front_mask_exact = nodes[:, 2] <= (torch.min(nodes[:, 2]) + tol_z)
    ...
    def _assembled_equilibrium_residual(self, stress_gp, Ele_info, EleConn):
        ...
        # reactions are penalized as free-DOF residuals.
        mesh_cache = getattr(self, '_primary_mesh_cache', None)
        ...
        if mesh_cache is not None:
            ...
            tol_y = torch.clamp(y_max - y_min, min=1e-12) * 1e-6
        ...
        try:
            ...
            try:
                ...
                if eligible_face_count > 0 and face_area_integrated > 0.0:
                    ...
                    s22_face_mean = s22_area_sum / face_area_integrated
            ...
            with open(out_csv, 'a', newline='') as fcsv:
                ...
                if write_header:
                    ...
                    w.writerow(['step', 'top_disp', 'top_disp_uni', 's22_volmean', 'top_s33_uni', 'top_reaction_force', 's22_from_reaction', 'A_top_face', 'top_reaction_energy', 'top_reaction_face', 'top_reaction_asm'])
