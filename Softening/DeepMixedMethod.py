...
try:
    DP_TANGENT_MODE = str(os.getenv('DP_TANGENT_MODE', 'consistent')).strip().lower()
except Exception:
    DP_TANGENT_MODE = 'consistent'
if DP_TANGENT_MODE not in ('consistent', 'elastic', 'symmetric'):
    logger.warning(f"Unknown DP_TANGENT_MODE='{DP_TANGENT_MODE}', falling back to 'consistent'.")
    DP_TANGENT_MODE = 'consistent'


def _signed_floor_preserve_sign(value, floor):
    """Clamp tiny divisors without flipping their sign."""
    if isinstance(value, torch.Tensor):
        floor_tensor = torch.as_tensor(floor, dtype=value.dtype, device=value.device)
        sign = torch.where(value < 0, -torch.ones_like(value), torch.ones_like(value))
        sign = torch.where(value == 0, torch.ones_like(value), sign)
        return torch.where(torch.abs(value) < floor_tensor, sign * floor_tensor, value)
    return floor if abs(value) < floor and value >= 0 else (-floor if abs(value) < floor else value)
...
class DruckerPragerPlasticity(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, strain, eps_p_old, PEEQ_old, D_tensor, G, K, TAN_BETA, TAN_PSI, cohesion_d, H, training_mode=True):
        ...
        # torch.autograd.Function.forward runs with grad disabled internally, even
        # during loss.backward(). Use an explicit caller-supplied mode instead.
        is_training = bool(training_mode)
        ...
        d_eff_old = dp_softening_cohesion(PEEQ_old)
        ...
        if plastic_mask.any():
            ...
            PEEQ_old_p = PEEQ_old[plastic_mask]
            ...
            denom_safe = _signed_floor_preserve_sign(denom, 1e-12)
            ...
            if True:
                ...
                def _compute_update(gamma_vals):
                    # For ψ≈0 the smooth-cone flow potential is g=q, so the
                    # equivalent plastic strain is tied to the deviatoric return.
                    # Once q is exhausted, additional apex/tensile-cutoff
                    # correction must not be counted as PEEQ-driven softening.
                    if psi_small:
                        gamma_apex_limit = torch.clamp(
                            t_trial_raw_p / (3.0 * G),
                            min=0.0
                        )
                        gamma_state = torch.minimum(gamma_vals, gamma_apex_limit)
                    else:
                        gamma_state = gamma_vals

                    q_new = torch.clamp(t_trial_raw_p - 3.0 * G * gamma_state, min=0.0)
                    apex_mask_local = q_new < APEX_TOL
                    # Use the actual tabular softening law at the updated history.
                    # A local d_old + H_old * Δγ approximation is only valid inside
                    # one table segment and causes stress/PEEQ inconsistency when a
                    # load increment crosses a softening breakpoint.
                    peeq_trial = PEEQ_old_p + gamma_state
                    d_new = dp_softening_cohesion(peeq_trial).to(device=device, dtype=dtype)
                    H_new = dp_softening_hardening_modulus(peeq_trial).to(device=device, dtype=dtype)
                    if psi_small:
                        p_smooth = p_trial_p
                    else:
                        p_smooth = p_trial_p + K * gamma_state * TAN_PSI
                    tan_beta_safe = _signed_floor_preserve_sign(TAN_BETA, 1e-12)
                    p_apex = -d_new / tan_beta_safe
                    p_new = torch.where(apex_mask_local, p_apex, p_smooth)
                    ...
                    delta_eps_local = gamma_state.unsqueeze(-1).unsqueeze(-1) * M_dir

                    # At the cone apex, the admissible DP stress is hydrostatic:
                    # q=0 and p=-d/tan(beta). Rebuild eps_p from elastic compliance so
                    # stress and eps_p remain mutually consistent for energy/force output.
                    # For ψ≈0, PEEQ is still updated by gamma_state above, not by this
                    # volumetric endpoint correction.
                    tr_sigma = torch.einsum('...ii->...', stress_local)
                    s_dev_local = stress_local - (tr_sigma / 3.0).unsqueeze(-1).unsqueeze(-1) * I_tensor
                    eps_el_local = (
                        s_dev_local / (2.0 * G)
                        + (tr_sigma / (9.0 * K)).unsqueeze(-1).unsqueeze(-1) * I_tensor
                    )
                    delta_eps_consistent = strain[plastic_mask] - eps_el_local - eps_p_old[plastic_mask]
                    delta_eps_local = torch.where(
                        apex_selector,
                        delta_eps_consistent,
                        delta_eps_local
                    )

                    f_resid, _, _ = dp_yield_function(stress_local, TAN_BETA, d_new)
                    # Exact projected apex stress has q=0, but dp_invariants
                    # adds eps_global under sqrt and reports a numerical q
                    # floor. Do not let that artificial residual drive Newton.
                    f_resid = torch.where(apex_mask_local, torch.zeros_like(f_resid), f_resid)
                    return stress_local, delta_eps_local, apex_mask_local, f_resid, q_new, p_new, d_new, H_new
                ...
                if not is_training and not hasattr(DruckerPragerPlasticity, '_closedform_logged'):  # PATCH-40 FIX
                    stress_cf, _, _, f_cf, q_cf, p_cf, d_cf, H_cf = _compute_update(delta_gamma)
                ...
                stress_local, delta_eps_local, apex_mask, f_resid, q_new, p_new, d_new, H_current = _compute_update(delta_gamma)
                t_cf_done = time.perf_counter() if is_training else None
                n_apex_triggered = int(apex_mask.sum().item())
                n_total_plastic = int(plastic_mask.sum().item())
                current_step = getattr(DruckerPragerPlasticity, '_current_step', None)
                if not hasattr(DruckerPragerPlasticity, '_apex_stats'):
                    DruckerPragerPlasticity._apex_stats = {}
                apex_stats = DruckerPragerPlasticity._apex_stats
                if current_step is not None:
                    stats = apex_stats.setdefault(current_step, {
                        'calls': 0,
                        'calls_with_apex': 0,
                        'apex_points_total': 0,
                        'plastic_points_total': 0,
                        'max_apex_points': 0,
                        'first_logged': False,
                    })
                    stats['calls'] += 1
                    stats['apex_points_total'] += n_apex_triggered
                    stats['plastic_points_total'] += n_total_plastic
                    stats['max_apex_points'] = max(stats['max_apex_points'], n_apex_triggered)
                    if n_apex_triggered > 0:
                        stats['calls_with_apex'] += 1
                    if (not stats['first_logged']) and n_apex_triggered > 0:
                        logger.info(
                            f"Step {current_step}: apex branch first triggered in this step ({n_apex_triggered} / {n_total_plastic} plastic points)"
                        )
                        stats['first_logged'] = True
                needs_refine = torch.abs(f_resid) > YIELD_TOL
                plastic_indices = torch.nonzero(plastic_mask, as_tuple=False).flatten()
                ...
                if needs_refine.any():  # Keep outer check (only 1 sync per forward pass)
                    ...
                    branch_history = [apex_mask.detach().clone()]
                    ...
                    for iter_idx in range(MAX_NEWTON_ITERS):
                        ...
                        denom_smooth = _signed_floor_preserve_sign(
                            3.0 * G + K * TAN_PSI * TAN_BETA + H_current,
                            1e-12
                        )
                        denom_apex = _signed_floor_preserve_sign(
                            K * TAN_PSI * TAN_BETA + H_current,
                            1e-12
                        )
                        ...
                        if psi_small:
                            zero_h_mask = apex_points & (torch.abs(H_current) <= H_ZERO_TOL)
                        ...
                        deriv = _signed_floor_preserve_sign(deriv, 1e-12)
                        ...
                        stress_local, delta_eps_local, apex_mask, f_resid, q_new, p_new, d_new, H_current = _compute_update(delta_gamma)
                        branch_history.append(apex_mask.detach().clone())
                        needs_refine = torch.abs(f_resid) > YIELD_TOL
                    ...
                    if branch_history:
                        branch_stack = torch.stack(branch_history, dim=0)
                        if branch_stack.shape[0] > 1:
                            flips = (branch_stack[1:] != branch_stack[:-1]).sum(dim=0)
                            max_flips = int(flips.max().item()) if flips.numel() > 0 else 0
                            mean_flips = float(flips.double().mean().item()) if flips.numel() > 0 else 0.0
                            n_flip_heavy = int((flips > 5).sum().item()) if flips.numel() > 0 else 0
                            max_abs_resid = float(torch.abs(f_resid).max().item()) if f_resid.numel() > 0 else 0.0
                            step_label = int(current_step) if current_step is not None else -1
                            if not hasattr(DruckerPragerPlasticity, '_apex_flip_stats'):
                                DruckerPragerPlasticity._apex_flip_stats = {}
                            flip_stats = DruckerPragerPlasticity._apex_flip_stats
                            step_flip = flip_stats.setdefault(step_label, {
                                'calls': 0,
                                'max_flips': 0,
                                'sum_flips': 0.0,
                                'n_points': 0,
                            ...
                            step_flip['calls'] += 1
                            step_flip['max_flips'] = max(step_flip['max_flips'], max_flips)
                            step_flip['sum_flips'] += mean_flips * flips.numel()
                            step_flip['n_points'] += flips.numel()
                            step_flip['n_flip_heavy'] += n_flip_heavy
                            step_flip['max_abs_resid'] = max(step_flip['max_abs_resid'], max_abs_resid)
                            if max_abs_resid > 1e-3 and flips.numel() > 0 and not step_flip['topk_logged']:
                                logger.info(
                                    f"Step {step_label}: apex-branch flips max={max_flips}, mean={mean_flips:.2f}, >5 flips={n_flip_heavy}/{flips.numel()}, final max|f|={max_abs_resid:.3e} MPa"
                                )
                                topk = min(5, flips.numel())
                                resid_topk = torch.topk(torch.abs(f_resid), k=topk)
                                for rank in range(topk):
                                    local_idx = int(resid_topk.indices[rank].item())
                                    global_idx = int(plastic_indices[local_idx].item()) if local_idx < plastic_indices.numel() else local_idx
                                    logger.info(
                                        f"  rank {rank + 1}: gp={global_idx}, |f|={float(torch.abs(f_resid[local_idx]).item()):.3e}, flips={int(flips[local_idx].item())}, apex_hits={int(branch_stack[:, local_idx].sum().item())}"
                                    )
                                step_flip['topk_logged'] = True
                ...
                if psi_small:
                    # Apply the deviatoric-flow history cap before any inference
                    # guardrails/logging so PEEQ growth diagnostics see the
                    # actual history increment used by the final state.
                    gamma_apex_limit = torch.clamp(t_trial_raw_p / (3.0 * G), min=0.0)
                    delta_gamma = torch.minimum(delta_gamma, gamma_apex_limit)
                    stress_local, delta_eps_local, apex_mask, f_resid, q_new, p_new, d_new, H_current = _compute_update(delta_gamma)

                # Preserve the refined Newton solution for the inference-only guardrails below.
                delta_gamma_refined = delta_gamma.clone()
                ...
                if PEEQ_old.numel() > 0 and not is_training:  # PATCH-40 FIX
                    ...
                    if is_perfect_plasticity:
                        # True H≈0 plateau: keep the Newton-refined update, even on the first plastic step.
                        if max_peeq_increment > 1e-2 and not hasattr(DruckerPragerPlasticity, '_peeq_first_warn'):
                            logger.info(
                                f"First plastic step on H≈0 plateau: PEEQ increment = {max_peeq_increment:.3e} "
                                f"(no clamping for ideal plasticity)"
                            )
                            DruckerPragerPlasticity._peeq_first_warn = True
                        stress_local, delta_eps_local, apex_mask, f_resid, q_new, p_new, d_new, H_current = _compute_update(delta_gamma)
                    else:
                        # Hardening/softening plasticity: apply growth-based clamping for stability,
                        # including the first plastic step.
                        ...
                        if max_peeq_increment > max_allowed_increment:
                            ...
                            if DruckerPragerPlasticity._peeq_warn_count == 0:
                                ...
                                if not getattr(DruckerPragerPlasticity, '_in_dense_vtk', False):
                                    logger.warning(
                                        f"PEEQ increment limited: Δγ={max_peeq_increment:.3e} "
                                        f"(ratio {growth_ratio:.2e}x, prev={max_peeq_old:.3e}) "
                ...
                # Explicit training_mode ensures this skips during optimization.
                # Only executes during dense mesh inference and SaveData (where output quality matters)
                if not is_training:
                    current_step = getattr(DruckerPragerPlasticity, '_current_step', None)
                    yield_step_key = current_step if current_step is not None else '__global__'
                    if not hasattr(DruckerPragerPlasticity, '_yield_projection_logged_step'):
                        DruckerPragerPlasticity._yield_projection_logged_step = None
                    f_check = f_resid
                    max_f_violation = torch.max(torch.abs(f_check)).item()

                    if max_f_violation > YIELD_TOL:
                        # Step 1: restore the refined Newton solution (undo any inference-only caps) and recompute state
                        delta_gamma = delta_gamma_refined.clone()
                        stress_local, delta_eps_local, apex_mask, f_resid, q_new, p_new, d_new, H_current = _compute_update(delta_gamma)
                        ...
                        if max_f_violation <= YIELD_TOL:
                            if DruckerPragerPlasticity._yield_projection_logged_step != yield_step_key:
                        ...
                        DruckerPragerPlasticity._yield_projection_logged_step = yield_step_key
                # ============================================================================

                if psi_small and not is_training:
                    # Forced projection is inference-only and may update Δγ after
                    # the first cap. Re-apply the same history cap before storing
                    # state so every path uses the same PEEQ definition.
                    gamma_apex_limit = torch.clamp(t_trial_raw_p / (3.0 * G), min=0.0)
                    delta_gamma = torch.minimum(delta_gamma, gamma_apex_limit)
                    stress_local, delta_eps_local, apex_mask, f_resid, q_new, p_new, d_new, H_current = _compute_update(delta_gamma)
        ...
        H_for_backward = getattr(ctx, 'H_algorithmic', None)
        if H_for_backward is not None:
            ctx.H = H_for_backward
    ...
    def backward(ctx, grad_stress, grad_eps_p, grad_peeq):
        ...
        if grad_eps_p is not None:
            grad_eps_p = grad_eps_p.double()
        if grad_peeq is not None:
            grad_peeq = grad_peeq.double()
        ...
        if grad_eps_p is None:
            grad_eps_p_batched = torch.zeros_like(strain_batched)
        elif unbatched_input and grad_eps_p.dim() == 2:
            grad_eps_p_batched = grad_eps_p.unsqueeze(0)
        else:
            grad_eps_p_batched = grad_eps_p

        if grad_peeq is None:
            grad_peeq_batched = torch.zeros(batch_size, dtype=dtype, device=device)
        else:
            grad_peeq_batched = grad_peeq.reshape(-1)

        tangent_modulus = D_tensor.unsqueeze(0).repeat(batch_size, 1, 1, 1, 1).contiguous()
        plastic_mask_index = plastic_mask.to(device=device, dtype=torch.bool).reshape(-1)
        ...
        K_safe = torch.clamp(torch.as_tensor(K, dtype=dtype, device=device), min=1e-12)
        G_safe = torch.clamp(torch.as_tensor(G, dtype=dtype, device=device), min=1e-12)
        D_inv_tensor = (1.0 / (9.0 * K_safe)) * I_x_I + (1.0 / (2.0 * G_safe)) * I_dev
        grad_eps_p_term_batched = torch.zeros_like(strain_batched)
        grad_peeq_term_batched = torch.zeros_like(strain_batched)

        if plastic_mask_index.any() and DP_TANGENT_MODE != 'elastic':
            ...
            if s_trial_p is not None:
                ...
                try:
                    D_ep_p = torch.zeros_like(tangent_modulus[plastic_mask_index])
                    # If mismatch vs stored per-point tensors, fall back to explicit shape init
                    if D_ep_p.dim() != 5 or D_ep_p.shape[0] != s_trial_p.shape[0]:
                ...
                peeq_grad_p = torch.zeros((s_trial_p.shape[0], 3, 3), dtype=tangent_modulus.dtype, device=tangent_modulus.device)
                ...
                if smooth_mask.any():
                    ...
                    # >>>>> FIX 2 & 3: Robust sign-preserving stabilization <<<<<
                    # Softening may produce negative H. Preserve the sign of the tangent denominator
                    # instead of flipping it to a positive floor, otherwise the CAT direction is wrong.
                    Denom_CAT_safe = _signed_floor_preserve_sign(Denom_CAT, 1e-12)
                    # >>>>> END OF FIX 2 & 3 <<<<<
                    ...
                    dgamma_dstrain_smooth = N_CeBar / Denom_CAT_safe.reshape(-1, 1, 1)
                    peeq_grad_p[smooth_mask] = dgamma_dstrain_smooth
                ...
                if apex_mask.any():
                    ...
                    # At the apex the forward stress is hydrostatic:
                    #   p = -d(kappa)/tan(beta), sigma = d(kappa)/tan(beta) I.
                    # Therefore the apex stress tangent is controlled by the history
                    # slope H=dd/dkappa, not by a purely elastic K I⊗I response.
                    # The plastic multiplier sensitivity follows the same trial-yield
                    # residual used by the closed-form predictor:
                    #   dgamma = (dq_trial - tan(beta) dp_trial) /
                    #            (3G + K tan(psi) tan(beta) + H).
                    apex_trial_s = s_trial_p[apex_mask]
                    apex_trial_q = t_trial_p[apex_mask]
                    N_flow_apex = (3.0 / 2.0) * (
                        apex_trial_s / apex_trial_q.unsqueeze(-1).unsqueeze(-1)
                    )
                    q_grad_apex = torch.einsum('pij,ijkl->pkl', N_flow_apex, D_tensor)
                    p_grad = -(torch.einsum('ij,ijkl->kl', I_tensor, D_tensor) / 3.0)
                    try:
                        psi_small_bwd = abs(float(TAN_PSI.item() if isinstance(TAN_PSI, torch.Tensor) else TAN_PSI)) < 1e-12
                    except Exception:
                        psi_small_bwd = False
                    if psi_small_bwd:
                        # Forward caps apex history by the deviatoric exhaustion
                        # limit gamma=q_trial/(3G). Its derivative must not
                        # contain the hydrostatic yield term -tan(beta) dp.
                        denom_gamma_apex = _signed_floor_preserve_sign(3.0 * G, 1e-12)
                        dgamma_dstrain_apex = q_grad_apex / denom_gamma_apex.reshape(-1, 1, 1)
                    else:
                        yield_grad_apex = q_grad_apex - TAN_BETA * p_grad.unsqueeze(0)
                        denom_gamma_apex = _signed_floor_preserve_sign(
                            3.0 * G + K * TAN_PSI * TAN_BETA + H_apex,
                            1e-12
                        )
                        dgamma_dstrain_apex = yield_grad_apex / denom_gamma_apex.reshape(-1, 1, 1)

                    tan_beta_safe = _signed_floor_preserve_sign(TAN_BETA, 1e-12)
                    hydro_coeff = H_apex.reshape(-1, 1, 1, 1, 1) / tan_beta_safe
                    D_ep_apex = (
                        hydro_coeff
                        * I_tensor.reshape(1, 3, 3, 1, 1)
                        * dgamma_dstrain_apex.reshape(-1, 1, 1, 3, 3)
                    )
                    ...
                    if num_apex_points > 0:
                        ...
                        peeq_grad_p[apex_mask] = dgamma_dstrain_apex

                if DP_TANGENT_MODE == 'symmetric':
                    D_ep_p = 0.5 * (D_ep_p + D_ep_p.permute(0, 3, 4, 1, 2).contiguous())
                ...
                n_plastic = int(plastic_mask_index.sum().item())
                ...
                tangent_modulus[plastic_mask_index] = D_ep_p
                if grad_eps_p_batched is not None:
                    grad_eps_p_plastic = grad_eps_p_batched[plastic_mask_index]
                    if grad_eps_p_plastic.numel() > 0:
                        # Contract the 4th-order elastic compliance with the per-point ep tangent.
                        # Use a distinct batch index (`p`) to avoid colliding with tensor indices.
                        Dinv_Calg = torch.einsum('abmn,pmnij->pabij', D_inv_tensor, D_ep_p)
                        J_eps_p = I_sym.unsqueeze(0) - Dinv_Calg
                        grad_eps_p_term_batched[plastic_mask_index] = torch.einsum('pab,pabij->pij', grad_eps_p_plastic, J_eps_p)
                if grad_peeq_batched is not None:
                    grad_peeq_plastic = grad_peeq_batched[plastic_mask_index]
                    if grad_peeq_plastic.numel() > 0:
                        grad_peeq_term_batched[plastic_mask_index] = grad_peeq_plastic.reshape(-1, 1, 1) * peeq_grad_p

        # Compute gradient w.r.t. strain using the adjoint of the algorithmic tangent.
        # tangent_modulus stores C_ijkl = ∂σ_ij/∂ε_kl.  The backward contraction is
        # grad_ε_kl = grad_σ_ij C_ijkl, i.e. sum over the stress-index pair (i,j).
        # The old contraction used C_klij; that is only harmless for elastic/associated
        # major-symmetric tangents and is wrong for non-associated DP.
        grad_strain_batched = (tangent_modulus * grad_stress_batched[..., :, :, None, None]).sum(dim=(1, 2))
        grad_strain_batched = grad_strain_batched + grad_eps_p_term_batched + grad_peeq_term_batched
        ...
        if not hasattr(DruckerPragerPlasticity, '_backward_debug_logged'):
            ...
            logger.info(f"  DP_TANGENT_MODE={DP_TANGENT_MODE}")
            ...
            expected_00 = (tangent_modulus[0,0,0,0,0] * grad_stress_batched[0,0,0] +
                          tangent_modulus[0,1,1,0,0] * grad_stress_batched[0,1,1] +
                          tangent_modulus[0,2,2,0,0] * grad_stress_batched[0,2,2])
        ...
        try:
            if ENABLE_BACKWARD_DEBUG and plastic_mask_index.any():
                ...
                if should_log:
                    try:
                        D_ep_block = tangent_modulus[plastic_mask_index]
        ...
        return grad_strain, None, None, None, None, None, None, None, None, None, None
...
class DeepMixedMethod:
    ...
    5. HARDENING MODULUS MAPPING (Section 4.3):
       ✓ Unified _effective_cohesion() method: d(PEEQ) from the shared softening table
       ✓ HardeningModulus returns the same table's local slope dd/dPEEQ
       ✓ Same softening table used everywhere
    ...
    def __init__(self, model):
        self.S_Net   = model[0]
        # Keep the neural network in float32 on CUDA, while constitutive/energy
        # calculations remain in float64.
        self.physics_dtype = torch.float64
        self.net_dtype = torch.float32 if dev.type == 'cuda' else torch.float64
        self.S_Net = self.S_Net.to(device=dev, dtype=self.net_dtype)
        self.lr = model[1]
        self.applied_disp = 0.
        ...
        logger.info(f"Precision config: net_dtype={self.net_dtype}, physics_dtype={self.physics_dtype}")
        ...
        self.assembled_equilibrium_weight_first_plastic = self.assembled_equilibrium_weight
        self._assembled_equilibrium_active_weight = self.assembled_equilibrium_weight
        self.assembled_equilibrium_target_grad_ratio = 0.0
        self.assembled_equilibrium_weight_min = 0.0
        self.assembled_equilibrium_weight_max = float('inf')
        self.internal_energy_weight_elastic = 1.0
        self.internal_energy_weight_first_plastic = 1.0
        self.internal_energy_weight_plastic = 0.0
        self._internal_energy_active_weight = 1.0
        self.softening_dissipation_weight = 0.0
        self.softening_dissipation_weight_first_plastic = 0.0
        self._softening_dissipation_active_weight = self.softening_dissipation_weight
        self.continuation_weight_first_plastic = 0.0
        self.continuation_weight_plastic = 0.0
        self._continuation_active_weight = 0.0
        self.peeq_increment_trust_weight_first_plastic = 0.0
        self.peeq_increment_trust_weight_plastic = 0.0
        self.peeq_increment_trust_cap_factor = 2.0
        self.peeq_increment_trust_min_cap = 5e-5
        self.peeq_increment_trust_lower_factor = 0.25
        self._peeq_increment_trust_active_weight = 0.0
        self._peeq_increment_trust_cap = float('inf')
        self._peeq_increment_trust_target = None
        self._peeq_increment_trust_mask = None
        self._peeq_increment_trust_target_points = 0
        self.distributed_plastic_rho_tol = 5e-3
        self.distributed_plastic_p95_tol = 1e-5
        self.distributed_plastic_max_tol = 1e-4
        self.incipient_plasticity_weight_tol = float(os.environ.get('DP_INCIPIENT_PLASTICITY_WEIGHT_TOL', '1e-4'))
        self.incipient_plasticity_zone_tol = float(os.environ.get('DP_INCIPIENT_PLASTICITY_ZONE_TOL', '1e-3'))
        self.history_plastic_gate_tol = float(os.environ.get(
            'DP_HISTORY_PLASTIC_GATE_TOL',
            str(self.incipient_plasticity_weight_tol)
        ))
        self.softening_failfast_enabled = bool(int(os.environ.get(
            'DP_SOFTENING_FAILFAST',
            '0'
        )))
        self.softening_failfast_min_step = int(os.environ.get('DP_SOFTENING_FAILFAST_MIN_STEP', '4'))
        self.softening_failfast_ref_rel_tol = float(os.environ.get('DP_SOFTENING_FAILFAST_REF_REL', '0.12'))
        self.softening_failfast_force_jump_rel = float(os.environ.get('DP_SOFTENING_FAILFAST_FORCE_JUMP_REL', '0.15'))
        self.softening_failfast_force_jump_abs = float(os.environ.get('DP_SOFTENING_FAILFAST_FORCE_JUMP_ABS', '3.0'))
        self.softening_failfast_frozen_dpeeq_tol = float(os.environ.get('DP_SOFTENING_FAILFAST_FROZEN_DPEEQ', '1e-5'))
        self.softening_force_reference_csv = os.environ.get('DP_ABAQUS_FORCE_CSV', '').strip()
        self._softening_force_reference_cache = None
        self._softening_path_prev = None
        self._softening_failfast_pending = None
        self._step_peeq_initial_for_diagnostics = None
        ...
        if len(model) > 4 and isinstance(model[4], dict):
            ...
            self.assembled_equilibrium_weight_first_plastic = float(
                penalty_cfg.get('assembled_equilibrium_weight_first_plastic', self.assembled_equilibrium_weight_plastic)
            )
            ...
            self.assembled_equilibrium_target_grad_ratio = float(
                penalty_cfg.get('assembled_equilibrium_target_grad_ratio', self.assembled_equilibrium_target_grad_ratio)
            )
            self.assembled_equilibrium_weight_min = float(
                penalty_cfg.get('assembled_equilibrium_weight_min', self.assembled_equilibrium_weight_min)
            )
            self.assembled_equilibrium_weight_max = float(
                penalty_cfg.get('assembled_equilibrium_weight_max', self.assembled_equilibrium_weight_max)
            )
            self.internal_energy_weight_elastic = float(penalty_cfg.get('internal_energy_weight_elastic', 1.0))
            self.internal_energy_weight_first_plastic = float(
                penalty_cfg.get('internal_energy_weight_first_plastic', self.internal_energy_weight_first_plastic)
            )
            self.internal_energy_weight_plastic = float(
                penalty_cfg.get('internal_energy_weight_plastic', self.internal_energy_weight_plastic)
            )
            self._internal_energy_active_weight = self.internal_energy_weight_elastic
            self.softening_dissipation_weight = float(
                penalty_cfg.get('softening_dissipation_weight', self.softening_dissipation_weight)
            )
            self.softening_dissipation_weight_first_plastic = float(
                penalty_cfg.get('softening_dissipation_weight_first_plastic', self.softening_dissipation_weight_first_plastic)
            )
            self._softening_dissipation_active_weight = self.softening_dissipation_weight
            self.continuation_weight_first_plastic = float(
                penalty_cfg.get('continuation_weight_first_plastic', self.continuation_weight_first_plastic)
            )
            self.continuation_weight_plastic = float(
                penalty_cfg.get('continuation_weight_plastic', self.continuation_weight_plastic)
            )
            self._continuation_active_weight = 0.0
            self.peeq_increment_trust_weight_first_plastic = float(
                penalty_cfg.get(
                    'peeq_increment_trust_weight_first_plastic',
                    self.peeq_increment_trust_weight_first_plastic
                )
            )
            self.peeq_increment_trust_weight_plastic = float(
                penalty_cfg.get('peeq_increment_trust_weight_plastic', self.peeq_increment_trust_weight_plastic)
            )
            self.peeq_increment_trust_cap_factor = float(
                penalty_cfg.get('peeq_increment_trust_cap_factor', self.peeq_increment_trust_cap_factor)
            )
            self.peeq_increment_trust_min_cap = float(
                penalty_cfg.get('peeq_increment_trust_min_cap', self.peeq_increment_trust_min_cap)
            )
            self.peeq_increment_trust_lower_factor = float(
                penalty_cfg.get('peeq_increment_trust_lower_factor', self.peeq_increment_trust_lower_factor)
            )
            self.distributed_plastic_rho_tol = float(
                penalty_cfg.get('distributed_plastic_rho_tol', self.distributed_plastic_rho_tol)
            )
            self.distributed_plastic_p95_tol = float(
                penalty_cfg.get('distributed_plastic_p95_tol', self.distributed_plastic_p95_tol)
            )
            self.distributed_plastic_max_tol = float(
                penalty_cfg.get('distributed_plastic_max_tol', self.distributed_plastic_max_tol)
            )
            if 'load_axis' in penalty_cfg:
                self.load_axis = str(penalty_cfg.get('load_axis', '')).strip().lower()

        if not hasattr(self, 'load_axis'):
            self.load_axis = os.environ.get('DP_LOAD_AXIS', 'y').strip().lower()
        if self.load_axis not in ('x', 'y', 'z'):
            logger.warning(f"Unknown DP_LOAD_AXIS='{self.load_axis}', falling back to Y")
            self.load_axis = 'y'
        self.load_dim = {'x': 0, 'y': 1, 'z': 2}[self.load_axis]
        self.transverse_dims = tuple(i for i in range(3) if i != self.load_dim)
        self.load_axis_label = self.load_axis.upper()
        self.stress_axis_label = f"S{self.load_dim + 1}{self.load_dim + 1}"
        self.disp_axis_label = f"U{self.load_axis_label.lower()}"
        logger.info(
            f"Load-axis configuration: axis={self.load_axis_label}, "
            f"disp={self.disp_axis_label}, stress={self.stress_axis_label}"
        )
        ...
        try:
            # The elastic hole perturbation is O(applied displacement).  Using
            # 0.1*disp as the correction scale traps the trial space near the
            # uniform no-hole hint field and over-predicts the elastic stiffness.
            self.nn_disp_scale = float(os.environ.get('NN_DISP_SCALE', '1.0'))
        except Exception:
            self.nn_disp_scale = 1.0
        try:
            # Softening in the plate-with-hole benchmark must be able to
            # localize around the stress concentrator.  Keep the enrichment
            # dormant during the elastic steps so the good elastic slope is not
            # perturbed, then activate it once the plastic gate opens.
            default_hole_enrich = '3.0' if getattr(self, 'load_axis', 'y') == 'y' else '0.0'
            self.hole_enrich_strength = float(os.environ.get('DP_HOLE_ENRICH_STRENGTH', default_hole_enrich))
        except Exception:
            self.hole_enrich_strength = 3.0
        try:
            self.hole_enrich_plastic_only = bool(int(os.environ.get('DP_HOLE_ENRICH_PLASTIC_ONLY', '1')))
        except Exception:
            self.hole_enrich_plastic_only = True
        self._hole_enrich_active = not self.hole_enrich_plastic_only
        try:
            self.hole_enrich_radius = float(os.environ.get('DP_HOLE_ENRICH_RADIUS_NORM', '0.125'))
        except Exception:
            self.hole_enrich_radius = 0.125
        try:
            self.hole_enrich_sigma = float(os.environ.get('DP_HOLE_ENRICH_SIGMA_NORM', '0.075'))
        except Exception:
            self.hole_enrich_sigma = 0.075
        try:
            self.equilibrium_topk_fraction = float(os.environ.get('DP_EQ_TOPK_FRAC', '0.0'))
        except Exception:
            self.equilibrium_topk_fraction = 0.0
        try:
            self.equilibrium_topk_weight = float(os.environ.get('DP_EQ_TOPK_WEIGHT', '0.0'))
        except Exception:
            self.equilibrium_topk_weight = 0.0
        logger.info(
            f"Displacement/equilibrium controls: NN_DISP_SCALE={self.nn_disp_scale:.3g}, "
            f"DP_HOLE_ENRICH_STRENGTH={self.hole_enrich_strength:.3g}, "
            f"DP_HOLE_ENRICH_PLASTIC_ONLY={int(self.hole_enrich_plastic_only)}, "
            f"DP_HOLE_ENRICH_RADIUS_NORM={self.hole_enrich_radius:.3g}, "
            f"DP_HOLE_ENRICH_SIGMA_NORM={self.hole_enrich_sigma:.3g}, "
            f"DP_EQ_TOPK_FRAC={self.equilibrium_topk_fraction:.3g}, "
            f"DP_EQ_TOPK_WEIGHT={self.equilibrium_topk_weight:.3g}"
        )
        logger.info(
            f"Softening path diagnostics: failfast={int(self.softening_failfast_enabled)}, "
            f"min_step={self.softening_failfast_min_step}, "
            f"ref_rel_tol={self.softening_failfast_ref_rel_tol:.3g}, "
            f"history_plastic_gate={self.history_plastic_gate_tol:.3g}, "
            f"incipient_zone_gate={self.incipient_plasticity_zone_tol:.3g}, "
            f"frozen_dPEEQ_tol={self.softening_failfast_frozen_dpeeq_tol:.3g}, "
            f"force_jump_abs={self.softening_failfast_force_jump_abs:.3g}, "
            f"force_jump_rel={self.softening_failfast_force_jump_rel:.3g}, "
            f"ref_csv={self.softening_force_reference_csv or 'built-in Abaqus table'}"
        )
        ...
        if self.bottom_center is None:
            raise RuntimeError(f"No bottom nodes found for load axis {self.load_axis_label}")
        self.top_center = primary_cache.get('top_center')
        if self.top_center is None:
            raise RuntimeError(f"No top nodes found for load axis {self.load_axis_label}")

        logger.info(f"Bottom center node index ({self.load_axis_label}=min face): {self.bottom_center}, coords: {nodesEn[self.bottom_center].cpu().tolist()}")
        logger.info(f"Top center node index ({self.load_axis_label}=max face): {self.top_center}, coords: {nodesEn[self.top_center].cpu().tolist()}")
        ...
        # Calculate and store the transverse center axis for the elastic hint field.
        bottom_coords = nodesEn[self.bottom_center]
        top_coords = nodesEn[self.top_center]
        trans0, trans1 = self.transverse_dims
        self.center_transverse = torch.stack([
            (bottom_coords[trans0] + top_coords[trans0]) / 2.0,
            (bottom_coords[trans1] + top_coords[trans1]) / 2.0,
        ]).to(dtype=torch.float64, device=self.dev)
        if self.load_axis == 'y':
            self.center_xz = self.center_transverse
        elif self.load_axis == 'z':
            self.center_xy = self.center_transverse
        logger.info(
            f"Calculated transverse center axis ({'XYZ'[trans0]}, {'XYZ'[trans1]}): "
            f"{self.center_transverse.cpu().tolist()}"
        )
        ...
        if not hasattr(self, 'inference_mesh_filename'):
            inference_mesh_env = os.environ.get('DP_INFERENCE_MESH', '').strip()
            self.inference_mesh_filename = inference_mesh_env or ('Hole.inp.denseM2' if self.load_axis == 'y' else None)
        ...
        if self.EXAMPLE in [4,5]:
            ...
            if hasattr(self, 'cohesion_d'):
                ...
                dilation_val = float(self.DILATION_ANGLE.item() if isinstance(self.DILATION_ANGLE, torch.Tensor) else self.DILATION_ANGLE)
                friction_val = float(self.FRICTION_ANGLE.item() if isinstance(self.FRICTION_ANGLE, torch.Tensor) else self.FRICTION_ANGLE)
                flow_label = 'ASSOCIATED' if abs(dilation_val - friction_val) <= 1e-12 else 'NON-ASSOCIATED'
                logger.info(f"Dilation Angle: {self.DILATION_ANGLE}° ({flow_label} FLOW)")
    ...
    def _hex8_face_definitions(self):
        """Return HEX8 face node sets and their fixed natural-coordinate axis."""
        if self.nodes_per_cell != 8:
            raise RuntimeError("HEX8 face definitions requested but element type is not HEX8")
        return [
            ((0, 1, 2, 3), 'zeta', -1.0),  # zeta-
            ((4, 5, 6, 7), 'zeta', 1.0),   # zeta+
            ((0, 1, 5, 4), 'eta', -1.0),   # eta-
            ((1, 2, 6, 5), 'xi', 1.0),     # xi+
            ((2, 3, 7, 6), 'eta', 1.0),    # eta+
            ((3, 0, 4, 7), 'xi', -1.0),    # xi-
    ...
    def _hex8_face_quadrature(self, face_idx: int, surface_order: int = 2):
        """Return quadrature points for a HEX8 face in natural coordinates."""
        face_defs = self._hex8_face_definitions()
        if face_idx < 0 or face_idx >= len(face_defs):
            raise IndexError(f"Invalid HEX8 face index: {face_idx}")

        _, fixed_axis, fixed_value = face_defs[face_idx]
        base_g = 1.0 / math.sqrt(3.0)
        if surface_order == 2:
            pts_1d = [-base_g, base_g]
            wts_1d = [1.0, 1.0]
        elif surface_order == 3:
            pts_1d = [-math.sqrt(3.0 / 5.0), 0.0, math.sqrt(3.0 / 5.0)]
            wts_1d = [5.0 / 9.0, 8.0 / 9.0, 5.0 / 9.0]
        else:
            raise ValueError(f"Unsupported face_integration_order={surface_order}; expected 2 or 3")

        face_specs = []
        for a, wa in zip(pts_1d, wts_1d):
            for b, wb in zip(pts_1d, wts_1d):
                xi = 0.0
                eta = 0.0
                zeta = 0.0
                if fixed_axis == 'xi':
                    ...
                    eta = b
                    zeta = fixed_value
                face_specs.append((xi, eta, zeta, wa * wb))
        return face_specs

    def _get_top_surface_node_mask(self, nodes: Optional[torch.Tensor] = None):
        """Return the cached top-surface node mask when available."""
        mesh_cache = getattr(self, '_primary_mesh_cache', None)
        if mesh_cache is not None and 'top_mask' in mesh_cache:
            return mesh_cache['top_mask']
        if nodes is None:
            return None
        bb = self.domain.get('BB', None) if isinstance(getattr(self, 'domain', None), dict) else None
        axis_dim = int(getattr(self, 'load_dim', 1))
        if bb is not None and len(bb) > axis_dim:
            axis_top = torch.min(nodes[:, axis_dim]) + torch.as_tensor(bb[axis_dim], dtype=nodes.dtype, device=nodes.device)
        else:
            axis_top = torch.max(nodes[:, axis_dim])
        tol = max(1e-6, 1e-8 * max(abs(float(axis_top.item())), 1.0))
        return torch.isclose(nodes[:, axis_dim], axis_top, atol=tol)
    ...
    def _set_training_sync_suppression(self, enable, intervals=None):
        ...
        if intervals is None:
            intervals = {'step': 1, 'adam': 2000, 'lbfgs': 200, 'grad': 2000}
    ...
    # Prepare checkpoint directory for robust weight transfer between steps
        ...
        top_history_csv = os.path.join(self.base, 'top_surface_reaction_displacement.csv')
        try:
            if os.path.exists(top_history_csv):
                os.remove(top_history_csv)
                logger.info(f"Cleared previous top surface reaction-displacement history: {top_history_csv}")
        except Exception as csv_clear_err:
            logger.warning(f"Could not clear previous top surface reaction-displacement history: {csv_clear_err}")
        ...
        # Apex-trigger logging is summarized per step to avoid repeated hot-loop logging.
        DruckerPragerPlasticity._apex_stats = {}
        ...
        for step in range(1, self.step_max+1):
            ...
            DruckerPragerPlasticity._current_step = step
            ...
            self._step_peeq_initial_for_diagnostics = PEEQ_initial.detach().clone()
            ...
            # ============================================================================
            # Plasticity gate for the equilibrium residual
            # ============================================================================
            # Localized plasticity around the hole is physical, not an outlier.  Once the
            # previous converged step has nonzero PEEQ history, the next incremental solve
            # must keep the stronger assembled-equilibrium residual active.
            # ============================================================================
            
            # Calculate plastic volume fraction
            plastic_threshold = 1e-6  # Lower threshold for detection
            if PEEQ_initial.numel() > 0:
                peeq_flat_initial = PEEQ_initial.flatten()
                max_peeq_initial = peeq_flat_initial.max().item()
                ...
                # Also check high-percentile history.  Plate-with-hole
                # plasticity is localized, so p95 can remain exactly zero even
                # after the hole-edge plastic zone is physically meaningful.
                p95_peeq = torch.quantile(peeq_flat_initial, 0.95).item()
            else:
                max_peeq_initial = 0.0
                rho_plast = 0.0
                p95_peeq = 0.0
            
            # Dynamic equilibrium penalty: enable in plastic regime仅在启用配点时才生效
            history_plastic_tol = float(getattr(self, 'history_plastic_gate_tol', 1e-4))
            incipient_zone_tol = float(getattr(self, 'incipient_plasticity_zone_tol', 1e-3))
            history_peeq_present = max_peeq_initial > 1e-10
            history_plastic = (
                max_peeq_initial >= history_plastic_tol
                or rho_plast >= incipient_zone_tol
            )
            rho_gate = float(getattr(self, 'distributed_plastic_rho_tol', 5e-3))
            p95_gate = float(getattr(self, 'distributed_plastic_p95_tol', 1e-5))
            max_gate = float(getattr(self, 'distributed_plastic_max_tol', 1e-4))
            predictor_zone_tol = incipient_zone_tol
            predictor_peeq_max = 0.0
            predictor_rho = 0.0
            predictor_increment_active = False
            predictor_first_plastic = False
            peeq_predictor_increment_ref = None

            # Increment predictor: evaluate the current network at the new
            # prescribed displacement before training this step.  For the first
            # plastic step it selects the transition objective; for later
            # plastic steps it provides a lower trust bound so committed
            # plastic history cannot freeze while the force keeps increasing.
            if PEEQ_initial.numel() > 0:
                old_dp_training_mode_probe = getattr(self, '_dp_training_mode', True)
                try:
                    with torch.no_grad():
                        self._dp_training_mode = True
                        U_predictor = self.getUP(
                            nodesEn,
                            torch.tensor(float(self.applied_disp), dtype=torch.float64, device=self.dev)
                        )
                        _, _, _, PEEQ_predictor, _ = self.LE_Gauss(
                            U_predictor, nodesEn, self.domain['nE'], EleConn, Ele_info,
                            eps_p_initial, PEEQ_initial, alpha_initial,
                            OUTPUT=False, ElasticOnly=False, return_state=True
                        )
                        peeq_predictor_increment = torch.clamp(PEEQ_predictor - PEEQ_initial, min=0.0)
                        peeq_predictor_increment_ref = peeq_predictor_increment.detach().clone()
                        predictor_peeq_max = float(peeq_predictor_increment.max().item())
                        predictor_points = int((peeq_predictor_increment > 1e-5).sum().item())
                        predictor_total = int(peeq_predictor_increment.numel())
                        predictor_rho = predictor_points / predictor_total if predictor_total > 0 else 0.0
                        predictor_increment_active = (
                            predictor_peeq_max >= max_gate
                            or predictor_rho >= predictor_zone_tol
                        )
                        predictor_first_plastic = (not history_plastic) and predictor_increment_active
                except Exception as predictor_err:
                    logger.warning(
                        f"[Plastic predictor] Step {step}: trial return-map failed "
                        f"({predictor_err}); falling back to history-only gate"
                    )
                finally:
                    self._dp_training_mode = old_dp_training_mode_probe
                    try:
                        del U_predictor, PEEQ_predictor, peeq_predictor_increment
                    except Exception:
                        pass

                if self._should_log('step', step):
                    logger.info(
                        f"[Plastic predictor] Step {step}: trial_max_dPEEQ={predictor_peeq_max:.3e}, "
                        f"trial_zone={predictor_rho:.3%}, active={predictor_increment_active}, "
                        f"first_plastic={predictor_first_plastic}, "
                        f"max_gate={max_gate:.1e}, zone_gate={predictor_zone_tol:.2%}"
                    )

            localized_plastic = (max_peeq_initial > max_gate) and (rho_plast > rho_gate)
            broad_plastic = p95_peeq > p95_gate
            distributed_plastic = localized_plastic or broad_plastic
            plast_gate = history_plastic or distributed_plastic or predictor_first_plastic
            first_plastic_transition = predictor_first_plastic or (history_plastic and (not distributed_plastic))
            self._hole_enrich_active = (
                (not bool(getattr(self, 'hole_enrich_plastic_only', True)))
                or bool(plast_gate)
            )
            ...
            if first_plastic_transition:
                target_assembled_weight = float(
                    getattr(self, 'assembled_equilibrium_weight_first_plastic', self.assembled_equilibrium_weight_plastic)
                )
                target_internal_weight = float(
                    getattr(self, 'internal_energy_weight_first_plastic', self.internal_energy_weight_plastic)
                )
                target_dissipation_weight = float(
                    getattr(self, 'softening_dissipation_weight_first_plastic', self.softening_dissipation_weight)
                )
                target_continuation_weight = float(
                    getattr(self, 'continuation_weight_first_plastic', 0.0)
                )
                target_peeq_trust_weight = float(
                    getattr(self, 'peeq_increment_trust_weight_first_plastic', 0.0)
                )
            elif plast_gate:
                target_assembled_weight = self.assembled_equilibrium_weight_plastic
                target_internal_weight = self.internal_energy_weight_plastic
                target_dissipation_weight = self.softening_dissipation_weight
                target_continuation_weight = float(
                    getattr(self, 'continuation_weight_plastic', 0.0)
                )
                target_peeq_trust_weight = float(
                    getattr(self, 'peeq_increment_trust_weight_plastic', 0.0)
                )
            else:
                target_assembled_weight = self.assembled_equilibrium_weight_elastic
                target_internal_weight = self.internal_energy_weight_elastic
                target_dissipation_weight = 0.0
                target_continuation_weight = 0.0
                target_peeq_trust_weight = 0.0
            # In non-associated softening, stored energy is not a full
            # incremental potential.  Once committed plastic history exists,
            # scale the assembled virtual-work residual by gradient norm so it
            # constrains equilibrium without dominating the admissible branch.
            grad_ratio_target = float(getattr(self, 'assembled_equilibrium_target_grad_ratio', 0.0))
            if (
                (not first_plastic_transition)
                and plast_gate
                and grad_ratio_target > 0.0
                and target_internal_weight > 0.0
            ):
                probe_ok = False
                old_dp_training_mode_probe = getattr(self, '_dp_training_mode', True)
                try:
                    self._dp_training_mode = True
                    self.S_Net.zero_grad(set_to_none=True)
                    U_probe = self.getUP(
                        nodesEn,
                        torch.tensor(float(self.applied_disp), dtype=torch.float64, device=self.dev)
                    )
                    internal_probe, stress_probe, _, _, _ = self.LE_Gauss(
                        U_probe, nodesEn, self.domain['nE'], EleConn, Ele_info,
                        eps_p_initial, PEEQ_initial, alpha_initial,
                        OUTPUT=False, ElasticOnly=False, return_state=True
                    )
                    internal_probe_loss = float(target_internal_weight) * internal_probe
                    internal_probe_loss.backward(retain_graph=True)

                    grad_internal_sq = torch.tensor(0.0, dtype=torch.float64, device=self.dev)
                    for param in self.S_Net.parameters():
                        if param.grad is not None:
                            g = param.grad.detach().norm()
                            grad_internal_sq = grad_internal_sq + g * g
                    grad_internal_norm = torch.sqrt(grad_internal_sq)

                    self.S_Net.zero_grad(set_to_none=True)
                    eq_probe = self._assembled_equilibrium_residual(
                        stress_gp=stress_probe,
                        Ele_info=Ele_info,
                        EleConn=EleConn,
                    )
                    eq_probe.backward()
                    ...
                    grad_internal_val = float(grad_internal_norm.detach().item())
                    grad_eq_val = float(grad_eq_norm.detach().item())
                    internal_probe_val = float(internal_probe.detach().item())
                    eq_probe_val = float(eq_probe.detach().item())
                    if grad_internal_val > 1e-30 and grad_eq_val > 1e-30:
                        raw_weight = grad_ratio_target * grad_internal_val / grad_eq_val
                        w_min_cfg = float(getattr(self, 'assembled_equilibrium_weight_min', 0.0))
                        # In softening plasticity, equilibrium is a constraint:
                        # the adaptive scaler may increase the virtual-work
                        # weight, but must not weaken it below the nominal
                        # plastic value.
                        w_min = max(
                            w_min_cfg,
                            float(getattr(self, 'assembled_equilibrium_weight_plastic', w_min_cfg))
                        )
                        w_max = float(getattr(self, 'assembled_equilibrium_weight_max', float('inf')))
                        if w_max < w_min:
                            w_max = w_min
                        scaled_weight = min(max(raw_weight, w_min), w_max)
                        target_assembled_weight = scaled_weight
                        probe_ok = True
                        if self._should_log('step', step):
                            logger.info(
                                f"[Assembled Residual Scale] Step {step}: "
                                f"target_grad_ratio={grad_ratio_target:.3e}, "
                                f"|grad_internal|={grad_internal_val:.3e}, "
                                f"|grad_eq|={grad_eq_val:.3e}, raw_weight={raw_weight:.3e}, "
                                f"min_weight={w_min:.3e}, used_weight={target_assembled_weight:.3e}, "
                                f"internal={internal_probe_val:.3e}, eq_residual={eq_probe_val:.3e}"
                            )
                    elif self._should_log('step', step):
                        logger.info(
                            f"[Assembled Residual Scale] Step {step}: skipped because "
                            f"|grad_internal|={grad_internal_val:.3e}, |grad_eq|={grad_eq_val:.3e}; "
                            f"using fallback weight {target_assembled_weight:.3e}"
                        )
                except Exception as scale_err:
                    logger.warning(
                        f"[Assembled Residual Scale] Step {step}: gradient scaling failed "
                        f"({scale_err}); using fallback weight {target_assembled_weight:.3e}"
                    )
                finally:
                    self._dp_training_mode = old_dp_training_mode_probe
                    self.S_Net.zero_grad(set_to_none=True)
                    try:
                        del U_probe, internal_probe, stress_probe, internal_probe_loss, eq_probe
                    except Exception:
                        pass
                if (not probe_ok) and self._should_log('step', step):
                    logger.info(
                        f"[Assembled Residual Scale] Step {step}: fallback assembled weight "
                        f"{target_assembled_weight:.3e}"
                    )
            elif first_plastic_transition and self._should_log('step', step):
                logger.info(
                    f"[Assembled Residual Scale] Step {step}: keeping first-plastic virtual-work weight "
                    f"{target_assembled_weight:.3e} with internal_weight={target_internal_weight:.3e}; "
                    f"gradient scaling deferred until distributed plasticity"
                )

            target_peeq_trust_cap = float('inf')
            if target_peeq_trust_weight > 0.0:
                if peeq_predictor_increment_ref is not None:
                    cap_seed = max(float(predictor_peeq_max), 1e-12)
                else:
                    cap_seed = max(float(max_peeq_initial), 1e-12)
                target_peeq_trust_cap = max(
                    float(getattr(self, 'peeq_increment_trust_min_cap', 5e-5)),
                    float(getattr(self, 'peeq_increment_trust_cap_factor', 2.0)) * cap_seed
                )
            target_peeq_trust_points = 0
            target_peeq_trust_max = 0.0
            if (
                target_peeq_trust_weight > 0.0
                and peeq_predictor_increment_ref is not None
                and predictor_increment_active
            ):
                peeq_target_mask = peeq_predictor_increment_ref > 1e-5
                target_peeq_trust_points = int(peeq_target_mask.sum().item())
                target_peeq_trust_max = float(peeq_predictor_increment_ref.max().item())
                self._peeq_increment_trust_target = peeq_predictor_increment_ref
                self._peeq_increment_trust_mask = peeq_target_mask.detach().clone()
                self._peeq_increment_trust_target_points = target_peeq_trust_points
            else:
                self._peeq_increment_trust_target = None
                self._peeq_increment_trust_mask = None
                self._peeq_increment_trust_target_points = 0
            if self._should_log('step', step):
                phase = 'first-plastic-transition' if first_plastic_transition else ('plastic' if plast_gate else 'elastic')
                logger.info(
                    f"[Plastic gate] Step {step}: max_PEEQ0={max_peeq_initial:.3e}, "
                    f"rho(PEEQ>{plastic_threshold:.0e})={rho_plast:.3%}, p95={p95_peeq:.3e}, "
                    f"phase={phase}, internal_weight={target_internal_weight:.3e}, "
                    f"assembled_weight={target_assembled_weight:.3e}, "
                    f"dissipation_weight={target_dissipation_weight:.3e}, "
                    f"continuation_weight={target_continuation_weight:.3e}, "
                    f"peeq_trust_weight={target_peeq_trust_weight:.3e}, "
                    f"peeq_trust_cap={target_peeq_trust_cap:.3e}, "
                    f"peeq_trust_lower_factor={float(getattr(self, 'peeq_increment_trust_lower_factor', 0.25)):.3e}, "
                    f"peeq_trust_target_max={target_peeq_trust_max:.3e}, "
                    f"peeq_trust_target_points={target_peeq_trust_points}, "
                    f"history_peeq_present={history_peeq_present}, "
                    f"history_gate={history_plastic_tol:.1e}, "
                    f"incipient_zone_gate={incipient_zone_tol:.2%}, "
                    f"hole_enrich_active={bool(getattr(self, '_hole_enrich_active', False))}, "
                    f"hole_enrich_strength={float(getattr(self, 'hole_enrich_strength', 0.0)):.3g}, "
                    f"localized={localized_plastic}, broad={broad_plastic}, "
                    f"rho_gate={rho_gate:.3e}, max_gate={max_gate:.3e}"
                )
            prev_weight = getattr(self, '_assembled_equilibrium_active_weight', self.assembled_equilibrium_weight_elastic)
            prev_dissipation_weight = getattr(
                self, '_softening_dissipation_active_weight', self.softening_dissipation_weight
            )
            self._assembled_equilibrium_active_weight = target_assembled_weight
            self._internal_energy_active_weight = target_internal_weight
            self._softening_dissipation_active_weight = target_dissipation_weight
            self._continuation_active_weight = target_continuation_weight
            self._peeq_increment_trust_active_weight = target_peeq_trust_weight
            self._peeq_increment_trust_cap = target_peeq_trust_cap
            ...
            if abs(target_dissipation_weight - prev_dissipation_weight) > 1e-12 and self._should_log('step', step):
                phase = 'first-plastic-transition' if first_plastic_transition else ('plastic' if plast_gate else 'elastic')
                logger.info(
                    f"[Softening Dissipation] Using {phase} weight {target_dissipation_weight:.3e} "
                    f"(prev {prev_dissipation_weight:.3e})"
                )
            ...
            run_collocation_autotune = (
                (not getattr(self, 'disable_collocation', False))
                and self._equilibrium_penalty_on
                and step > 2
                and (not plast_gate)
                and float(getattr(self, '_internal_energy_active_weight', 0.0)) > 0.0
            )
            if run_collocation_autotune:
                try:
            ...
            elif self._equilibrium_penalty_on and plast_gate and self._should_log('step', step):
                logger.info(
                    "[Auto-tune B] Skipping legacy collocation gradient tuning in plastic regime; "
                    "assembled virtual-work residual governs weak equilibrium."
                )
            
            # Two-stage optimization parameters (per RevisionIdea.md)
            # CRITICAL FIX 5: Increase Adam epochs for better convergence
            ADAM_EPOCHS_INITIAL = int(os.environ.get('DP_ADAM_EPOCHS_INITIAL', '5000'))
            # FIX C: Increased subsequent epochs to stabilize the transition to plasticity
            ADAM_EPOCHS_SUBSEQUENT = int(os.environ.get('DP_ADAM_EPOCHS_SUBSEQUENT', '5000'))
            ...
            continuation_weight_value = float(getattr(self, '_continuation_active_weight', 0.0))
            if step > 1 and continuation_weight_value > 0.0:
                try:
                    current_disp_for_cont = float(self.applied_disp)
                    try:
                        previous_disp_for_cont = float(disp_schedule[step - 2])
                    except Exception:
                        previous_disp_for_cont = 0.0
                    continuation_scale = max(
                        abs(current_disp_for_cont),
                        abs(current_disp_for_cont - previous_disp_for_cont),
                        1e-6
                    )
                    with torch.no_grad():
                        continuation_ref = self.getUP(
                            nodesEn,
                            torch.tensor(current_disp_for_cont, dtype=torch.float64, device=self.dev)
                        ).detach()
                    if self._should_log('step', step):
                        logger.info(
                            f"[Continuation] Step {step}: active weight={continuation_weight_value:.3e}, "
                            f"scale={continuation_scale:.3e}. Reference is previous network evaluated at current displacement."
                        )
                except Exception as cont_err:
                    continuation_ref = None
                    logger.warning(
                        f"[Continuation] Step {step}: disabled because reference construction failed ({cont_err})"
                    )
            ...
            def calculate_loss(epoch_idx: int = 0):
                ...
                if continuation_ref is not None and continuation_weight_value > 0.0:
                    cont_residual = torch.mean(((U_full - continuation_ref) / continuation_scale) ** 2)
                    loss_core = loss_core + torch.tensor(
                        continuation_weight_value, dtype=torch.float64, device=self.dev
                    ) * cont_residual
                    if epoch_idx in (0, 2000, 4000):
                        if not hasattr(self, '_continuation_logged'):
                            self._continuation_logged = set()
                        cont_key = (int(step), int(epoch_idx))
                        if cont_key not in self._continuation_logged:
                            self._continuation_logged.add(cont_key)
                            logger.info(
                                f"[Continuation] Step {step} Epoch {epoch_idx}: "
                                f"residual={float(cont_residual.detach().item()):.6e}, "
                                f"loss={continuation_weight_value * float(cont_residual.detach().item()):.6e}"
                            )
            ...
            if first_plastic_transition:
                first_plastic_lr_scale = float(os.environ.get('DP_FIRST_PLASTIC_LR_SCALE', '0.05'))
                adam_lr = max(adam_lr * first_plastic_lr_scale, 1e-8)
                logger.info(
                    f"[First-plastic LR] Step {step}: scaled Adam LR by "
                    f"{first_plastic_lr_scale:.3e} -> {adam_lr:.3e}"
                )
            ...
            # 更保守的多阶段 LR 计划
            lr_floor_late = 1e-8 if first_plastic_transition else 2e-6
            lr_schedule_plan = [
                (0, min(3e-5, adam_lr * 2.0)),
                (1000, adam_lr),
                (4000, max(adam_lr * 0.5, lr_floor_late)),
            ]
            ...
            step_loss_rollback_tol = max(1.0, float(os.environ.get('DP_STEP_ROLLBACK_TOL', '1.02')))
            best_step_state = None
            best_step_loss_val = float('inf')
            best_step_label = 'uninitialized'
            skip_refine_phase_for_step = False

            def _clone_net_state():
                return {k: v.detach().clone() for k, v in self.S_Net.state_dict().items()}

            def _restore_net_state(state):
                if state is not None:
                    self.S_Net.load_state_dict(state)

            def _eval_current_step_loss(label):
                with torch.no_grad():
                    loss_eval = calculate_loss(epoch_idx=-1)
                loss_val = float(loss_eval.detach().item())
                if not math.isfinite(loss_val):
                    raise RuntimeError(f"non-finite loss during {label}: {loss_val}")
                return loss_val

            def _update_best_step_state(loss_val, label):
                nonlocal best_step_state, best_step_loss_val, best_step_label
                if math.isfinite(loss_val) and loss_val < best_step_loss_val:
                    best_step_loss_val = loss_val
                    best_step_state = _clone_net_state()
                    best_step_label = label

            try:
                initial_step_loss_val = _eval_current_step_loss('pre-Adam baseline')
                _update_best_step_state(initial_step_loss_val, 'pre-Adam')
                logger.info(
                    f"[Step best-state] Step {step}: baseline loss={initial_step_loss_val:.6e}, "
                    f"rollback_tol={step_loss_rollback_tol:.3f}"
                )
            except Exception as e_best_init:
                logger.warning(f"[Step best-state] Step {step}: failed to evaluate pre-Adam baseline: {e_best_init}")
            ...
            for adam_epoch in range(adam_epochs):
                ...
                # ============================================================================
                # ROOT CAUSE: the first plastic step needs the stronger equilibrium
                # residual as soon as return mapping detects plasticity. The previous
                # converged history variables must stay fixed inside the load step;
                # only the loss weights are allowed to change during optimization.
                # ============================================================================
                if adam_epoch + 1 == 2000 and not hasattr(self, '_plastic_corrector_applied'):
                    ...
                    with torch.no_grad():
                        ...
                        old_dp_training_mode = getattr(self, '_dp_training_mode', True)
                        self._dp_training_mode = True
                        try:
                            _, stress_mid, _, PEEQ_mid, _, _ = self.LE_Gauss(
                                U_mid, nodesEn, self.domain['nE'], EleConn, Ele_info,
                                eps_p_initial, PEEQ_initial, alpha_initial, OUTPUT=True, ElasticOnly=False
                            )
                        finally:
                            self._dp_training_mode = old_dp_training_mode
                        
                        # Check if plasticity has emerged
                        peeq_max_before = PEEQ_initial.max().item()
                        peeq_max_detected = PEEQ_mid.max().item()
                        
                        INCIPIENT_PLASTICITY_WEIGHT_TOL = float(
                            getattr(self, 'incipient_plasticity_weight_tol', 1e-4)
                        )
                        INCIPIENT_PLASTICITY_WARN_TOL = INCIPIENT_PLASTICITY_WEIGHT_TOL
                        INCIPIENT_PLASTICITY_ZONE_TOL = float(
                            getattr(self, 'incipient_plasticity_zone_tol', 1e-3)
                        )
                        if peeq_max_before < 1e-10 and peeq_max_detected > 1e-10:
                            peeq_increment = torch.clamp(PEEQ_mid - PEEQ_initial, min=0.0)
                            plastic_mask_diag = peeq_increment > 1e-5
                            plastic_points = int(plastic_mask_diag.sum().item())
                            total_points = int(peeq_increment.numel())
                            plastic_frac_ratio = plastic_points / total_points if total_points > 0 else 0.0
                            plastic_frac = 100.0 * plastic_frac_ratio
                            should_switch_weights = (
                                peeq_max_detected >= INCIPIENT_PLASTICITY_WEIGHT_TOL
                                or plastic_frac_ratio >= INCIPIENT_PLASTICITY_ZONE_TOL
                            )

                            if should_switch_weights:
                                # FIRST PLASTIC DETECTION: keep previous-step
                                # history fixed, but use a transition loss.  Do
                                # not jump directly to the fully plastic loss,
                                # because a few local return-mapped points are
                                # not a global softening branch.
                                old_asm_weight = float(getattr(self, '_assembled_equilibrium_active_weight', self.assembled_equilibrium_weight_elastic))
                                old_internal_weight = float(getattr(self, '_internal_energy_active_weight', self.internal_energy_weight_elastic))
                                old_diss_weight = float(getattr(self, '_softening_dissipation_active_weight', self.softening_dissipation_weight))
                                old_peeq_trust_weight = float(getattr(self, '_peeq_increment_trust_active_weight', 0.0))
                                first_plastic_weight = float(
                                    getattr(self, 'assembled_equilibrium_weight_first_plastic', self.assembled_equilibrium_weight_plastic)
                                )
                                first_internal_weight = float(
                                    getattr(self, 'internal_energy_weight_first_plastic', self.internal_energy_weight_plastic)
                                )
                                first_diss_weight = float(
                                    getattr(self, 'softening_dissipation_weight_first_plastic', self.softening_dissipation_weight)
                                )
                                first_peeq_trust_weight = float(
                                    getattr(self, 'peeq_increment_trust_weight_first_plastic', 0.0)
                                )
                                already_active = (
                                    abs(old_asm_weight - first_plastic_weight) <= 1e-12
                                    and abs(old_internal_weight - first_internal_weight) <= 1e-12
                                    and abs(old_diss_weight - first_diss_weight) <= 1e-12
                                    and abs(old_peeq_trust_weight - first_peeq_trust_weight) <= 1e-12
                                )
                                if already_active:
                                    self._hole_enrich_active = True
                                    logger.info(
                                        f"[PATCH-38-CHECK] Step {step} Epoch 2000: incipient plasticity already covered "
                                        f"by epoch-0 predictor. PEEQ_detected={peeq_max_detected:.6e}; "
                                        f"Plastic zone: {plastic_points}/{total_points} points ({plastic_frac:.2f}%)."
                                    )
                                else:
                                    self._assembled_equilibrium_active_weight = first_plastic_weight
                                    self._internal_energy_active_weight = first_internal_weight
                                    self._softening_dissipation_active_weight = first_diss_weight
                                    self._peeq_increment_trust_active_weight = first_peeq_trust_weight
                                    if first_peeq_trust_weight > 0.0:
                                        cap_seed = max(float(peeq_max_detected), 1e-12)
                                        self._peeq_increment_trust_cap = max(
                                            float(getattr(self, 'peeq_increment_trust_min_cap', 5e-5)),
                                            float(getattr(self, 'peeq_increment_trust_cap_factor', 2.0)) * cap_seed
                                        )
                                    self._hole_enrich_active = True
                                    ...
                                    msg = (
                                        f"[PATCH-38-CORRECTOR] Step {step} Epoch 2000: incipient plasticity detected mid-training. "
                                        f"PEEQ_before=0, PEEQ_detected={peeq_max_detected:.6e}. "
                                        f"Kept previous-step history fixed; activated first-plastic transition weights for remaining epochs. "
                                        f"Plastic zone: {plastic_points}/{total_points} points ({plastic_frac:.2f}%). "
                                        f"Internal energy weight {old_internal_weight:.3e}->{self._internal_energy_active_weight:.3e}; "
                                        f"assembled equilibrium weight {old_asm_weight:.3e}->{self._assembled_equilibrium_active_weight:.3e}; "
                                        f"dissipation weight {old_diss_weight:.3e}->{self._softening_dissipation_active_weight:.3e}; "
                                        f"PEEQ trust weight {old_peeq_trust_weight:.3e}->{self._peeq_increment_trust_active_weight:.3e}; "
                                        f"PEEQ trust cap={float(getattr(self, '_peeq_increment_trust_cap', float('inf'))):.3e}; "
                                        f"hole_enrich_active={bool(getattr(self, '_hole_enrich_active', False))}"
                                    )
                                    if (
                                        peeq_max_detected >= INCIPIENT_PLASTICITY_WARN_TOL
                                        or plastic_frac_ratio >= INCIPIENT_PLASTICITY_ZONE_TOL
                                    ):
                                        logger.warning(msg)
                                    else:
                                ...
                                logger.info(
                                    f"[PATCH-38-CORRECTOR] Step {step} Epoch 2000: local incipient plasticity below transition threshold. "
                                    f"PEEQ_before=0, PEEQ_detected={peeq_max_detected:.6e}; "
                                    f"Plastic zone: {plastic_points}/{total_points} points ({plastic_frac:.2f}%). "
                                    f"Weights unchanged (PEEQ_weight_tol={INCIPIENT_PLASTICITY_WEIGHT_TOL:.1e}, "
                                    f"zone_tol={100.0 * INCIPIENT_PLASTICITY_ZONE_TOL:.2f}%)."
                                )
                        else:
                ...
                if (adam_epoch + 1) % 200 == 0 or adam_epoch == 0 or adam_epoch == adam_epochs - 1:
                    history_loss_val = float(total_loss.detach().item())  # CONFIG: deferred sync
                    try:
                        post_step_loss_val = _eval_current_step_loss(f"Adam epoch {adam_epoch + 1}")
                        history_loss_val = post_step_loss_val
                        _update_best_step_state(post_step_loss_val, f"Adam epoch {adam_epoch + 1}")
                    except Exception as e_best_adam:
                        logger.debug(f"[Step best-state] Step {step}: skipped Adam best check at epoch {adam_epoch + 1}: {e_best_adam}")
                    adam_loss_history.append(history_loss_val)
            ...
            try:
                adam_final_loss_val = _eval_current_step_loss('post-Adam')
                if (
                    best_step_state is not None
                    and math.isfinite(best_step_loss_val)
                    and adam_final_loss_val > best_step_loss_val * step_loss_rollback_tol
                ):
                    _restore_net_state(best_step_state)
                    logger.warning(
                        f"[Step rollback] Step {step}: Adam ended worse than best state "
                        f"({adam_final_loss_val:.6e} > {step_loss_rollback_tol:.3f} * "
                        f"{best_step_loss_val:.6e}). Restored {best_step_label}."
                    )
                    adam_loss_history.append(best_step_loss_val)
                else:
                    _update_best_step_state(adam_final_loss_val, 'post-Adam')
            except Exception as e_adam_final:
                logger.warning(f"[Step rollback] Step {step}: post-Adam loss check failed: {e_adam_final}")

            # If Adam failed, skip refine and restore the best accepted state for this step.
            if adam_failed:
                if best_step_state is not None:
                    _restore_net_state(best_step_state)
                    logger.warning(
                        f"Adam phase detected instability; restored {best_step_label} "
                        f"(loss={best_step_loss_val:.6e}) and skipping refine phase."
                    )
                else:
                    logger.warning("Adam phase detected instability; no best-state snapshot was available. Skipping refine phase.")
                skip_refine_phase_for_step = True
            ...
            if not bool(getattr(self, 'skip_lbfgs', False)) and not skip_lbfgs_due_to_high_loss:
                try:
                    ...
                    if (
                        pre_lbfgs_state is not None
                        and math.isfinite(pre_loss_val)
                        and math.isfinite(final_loss_val)
                        and final_loss_val > pre_loss_val * step_loss_rollback_tol
                    ):
                        _restore_net_state(pre_lbfgs_state)
                        logger.warning(
                            f"[Step rollback] Step {step}: L-BFGS ended worse than pre-L-BFGS "
                            f"({final_loss_val:.6e} > {step_loss_rollback_tol:.3f} * "
                            f"{pre_loss_val:.6e}). Restored pre-L-BFGS Adam state."
                        )
                        lbfgs_loss_history.append(pre_loss_val)
                    else:
                        _update_best_step_state(final_loss_val, 'post-L-BFGS')
                    logger.info(f"L-BFGS phase finished in {time.time() - t1:.2f} seconds. Final Loss: {final_loss_val:.6e}")
                    ...
                    if pre_lbfgs_state is not None:
                        _restore_net_state(pre_lbfgs_state)
                        logger.warning(f"[Step rollback] Step {step}: restored pre-L-BFGS state after L-BFGS failure.")
                finally:
            ...
            skip_refine_phase = skip_refine_phase or skip_refine_phase_for_step
            ...
            # Default for Example 5 debugging: keep post-processing even if the
            # sanity check flags a bad state, so all 20 VTK/CSV steps are emitted.
            if stress_explosion_detected:
                skip_after_explosion = bool(int(os.environ.get('DP_SKIP_STRESS_EXPLOSION_STEP', '0')))
                if skip_after_explosion:
                    logger.info(f"Skipping Step {step} due to stress explosion. Moving to next step.")
                    continue
                logger.warning(
                    f"Step {step} stress explosion was detected, but post-processing will continue "
                    f"because DP_SKIP_STRESS_EXPLOSION_STEP=0."
                )
            ...
            # Authoritative state update uses the training path, not inference-only guardrails.
            old_dp_training_mode = getattr(self, '_dp_training_mode', True)
            self._dp_training_mode = True
            try:
            ...
            if torch.cuda.is_available():
                ...
                try:
                    ...
                    # Detect the first committed plastic state.  The history is
                    # committed only after this converged step; the following step
                    # must start with the plastic equilibrium gate active.
                    if peeq_max_previous < 1e-10 and peeq_max_updated > 1e-10:
                        logger.info(
                            ...
                            f"History committed at step end; next step should log phase=plastic."
                        )
            ...
            Data = [final_strain.detach(), final_stress.detach(), eps_p, PEEQ, alpha, stress_gp_field.detach()]
            ...
            if self.enable_volume_guardrails and self._should_log('step', step, final=(step == self.step_max)):
                top_mask_debug = self._get_top_surface_node_mask(nodesEn)
                axis_coords_debug = nodesEn[:, self.load_dim]
                axis_min_debug = torch.min(axis_coords_debug)
                axis_span_debug = torch.clamp(torch.max(axis_coords_debug) - axis_min_debug, min=1e-12)
                bottom_mask_debug = self._primary_mesh_cache.get(
                    'bottom_mask_exact',
                    axis_coords_debug <= (axis_min_debug + axis_span_debug * 1e-6)
                )
                logger.info(f"Top loaded U{self.load_axis_label.lower()} mean: {u_pred[top_mask_debug, self.load_dim].mean().item()}")
                ...
                logger.info(f"Bottom Ux mean: {u_pred[bottom_mask_debug, 0].mean().item()}")
                logger.info(f"Bottom Uy mean: {u_pred[bottom_mask_debug, 1].mean().item()}")
                logger.info(f"Bottom Uz mean: {u_pred[bottom_mask_debug, 2].mean().item()}")
                ...
                logger.info(f"Top center Ux: {u_pred[self.top_center, 0].item():.6e}, Uy: {u_pred[self.top_center, 1].item():.6e}, Uz: {u_pred[self.top_center, 2].item():.6e}")
            ...
            if save_every_steps:
                try:
                    ...
                    should_save = True
            if should_save:
                continue_on_savedata_error = bool(int(os.environ.get(
                    'DP_CONTINUE_ON_SAVEDATA_ERROR',
                    '1' if int(self.EXAMPLE) == 5 else '0'
                )))
                try:
                    curr_diff = self.SaveData(self.domain, u_pred, Data, FinalState, combined_history, step, ref_file)
                    all_diff.append(curr_diff)
                except Exception as e_save:
                    if continue_on_savedata_error:
                        logger.error(
                            f"[SaveData Step {step}] failed but training will continue "
                            f"because DP_CONTINUE_ON_SAVEDATA_ERROR=1: {e_save}"
                        )
                        all_diff.append([])
                    else:
                        raise
            ...
            # Apex-trigger summary for this step, aggregated over all forward calls.
            apex_stats = getattr(DruckerPragerPlasticity, '_apex_stats', {})
            step_apex = apex_stats.get(step)
            if step_apex is not None:
                calls = step_apex['calls']
                calls_with_apex = step_apex['calls_with_apex']
                apex_points_total = step_apex['apex_points_total']
                plastic_points_total = step_apex['plastic_points_total']
                max_apex_points = step_apex['max_apex_points']
                if plastic_points_total > 0:
                    apex_ratio = 100.0 * apex_points_total / plastic_points_total
                else:
                    apex_ratio = 0.0
                if apex_points_total > 0:
                    logger.info(
                        f"[Step {step} apex summary] forward_calls={calls}, calls_with_apex={calls_with_apex}, "
                        f"apex_points_total={apex_points_total}, plastic_points_total={plastic_points_total}, "
                        f"max_apex_points_per_call={max_apex_points}, apex_ratio={apex_ratio:.3f}%"
                    )
                else:
                    logger.info(
                        f"[Step {step} apex summary] forward_calls={calls}, calls_with_apex=0, "
                        f"apex branch not triggered, plastic_points_total={plastic_points_total}"
                    )

            flip_stats = getattr(DruckerPragerPlasticity, '_apex_flip_stats', {})
            step_flip = flip_stats.get(step)
            if step_flip is not None and step_flip['n_points'] > 0:
                mean_flips = step_flip['sum_flips'] / step_flip['n_points']
                logger.info(
                    f"[Step {step} apex-flip summary] calls={step_flip['calls']}, "
                    f"max_flips={step_flip['max_flips']}, mean_flips={mean_flips:.2f}, "
                    f">5_flips={step_flip['n_flip_heavy']}/{step_flip['n_points']}, "
                    f"max|f|={step_flip['max_abs_resid']:.3e} MPa"
                )
            ...
            try:
                checkpoint_payload = {
                    ...
                    'step': int(step),
                    'applied_disp': float(self.applied_disp),
                    'eps_p': eps_p.detach().cpu().clone(),
                    'PEEQ': PEEQ.detach().cpu().clone(),
        ...
        apex_stats = getattr(DruckerPragerPlasticity, '_apex_stats', {})
        if apex_stats:
            total_calls = sum(v['calls'] for v in apex_stats.values())
            total_calls_with_apex = sum(v['calls_with_apex'] for v in apex_stats.values())
            total_apex_points = sum(v['apex_points_total'] for v in apex_stats.values())
            total_plastic_points = sum(v['plastic_points_total'] for v in apex_stats.values())
            max_apex_points = max(v['max_apex_points'] for v in apex_stats.values())
            if total_plastic_points > 0:
                total_apex_ratio = 100.0 * total_apex_points / total_plastic_points
            else:
                total_apex_ratio = 0.0
            logger.info(
                f"[Apex summary total] steps={len(apex_stats)}, forward_calls={total_calls}, "
                f"calls_with_apex={total_calls_with_apex}, apex_points_total={total_apex_points}, "
                f"plastic_points_total={total_plastic_points}, max_apex_points_per_call={max_apex_points}, "
                f"apex_ratio={total_apex_ratio:.3f}%"
            )

        flip_stats = getattr(DruckerPragerPlasticity, '_apex_flip_stats', {})
        if flip_stats:
            total_flip_calls = sum(v['calls'] for v in flip_stats.values())
            total_flip_points = sum(v['n_points'] for v in flip_stats.values())
            total_flip_heavy = sum(v['n_flip_heavy'] for v in flip_stats.values())
            global_max_flips = max(v['max_flips'] for v in flip_stats.values())
            global_max_resid = max(v['max_abs_resid'] for v in flip_stats.values())
            mean_flips_total = 0.0
            if total_flip_points > 0:
                mean_flips_total = sum(v['sum_flips'] for v in flip_stats.values()) / total_flip_points
            logger.info(
                f"[Apex flip summary total] steps={len(flip_stats)}, calls={total_flip_calls}, "
                f"max_flips={global_max_flips}, mean_flips={mean_flips_total:.2f}, "
                f">5_flips={total_flip_heavy}/{total_flip_points}, max|f|={global_max_resid:.3e} MPa"
            )
        ...
        try:
            ...
            if hasattr(torch, 'func'):
                ...
                def f_single(inp: torch.Tensor) -> torch.Tensor:
                    # inp: (3,), returns (3,)
                    inp_net = inp.to(dtype=self.net_dtype)
                    out_net = self.S_Net(inp_net.unsqueeze(0)).squeeze(0)
                    return out_net.to(dtype=self.physics_dtype)
    ...
    def _build_mesh_cache(self, domain):
        ...
        span_x = torch.clamp(torch.max(nodes[:, 0]) - torch.min(nodes[:, 0]), min=1e-12)
        span_y = torch.clamp(torch.max(nodes[:, 1]) - torch.min(nodes[:, 1]), min=1e-12)
        span_z = torch.clamp(torch.max(nodes[:, 2]) - torch.min(nodes[:, 2]), min=1e-12)
        tol_x = span_x * 1e-6
        tol_y = span_y * 1e-6
        tol_z = span_z * 1e-6

        def _mask_from_node_set(set_name):
            node_sets = domain.get('node_sets', {})
            if not isinstance(node_sets, dict):
                return None
            idx = node_sets.get(set_name.upper())
            if idx is None:
                return None
            idx = torch.as_tensor(idx, dtype=torch.long, device=self.dev)
            idx = idx[(idx >= 0) & (idx < nodes.shape[0])]
            if idx.numel() == 0:
                return None
            mask = torch.zeros(nodes.shape[0], dtype=torch.bool, device=self.dev)
            mask[idx] = True
            return mask

        axis_dim = int(getattr(self, 'load_dim', 1))
        tol_axis = [tol_x, tol_y, tol_z][axis_dim]
        axis_min = torch.min(nodes[:, axis_dim])
        physical_axis_top = axis_min + bb_tensor[axis_dim]

        bottom_mask_coord = nodes[:, axis_dim] <= (axis_min + tol_axis)
        left_mask_coord = nodes[:, 0] <= (torch.min(nodes[:, 0]) + tol_x)
        front_mask_coord = nodes[:, 2] <= (torch.min(nodes[:, 2]) + tol_z)
        # Abaqus decks may contain an external reference node outside the physical
        # domain (e.g. node 9000000 tied to TOP). The displacement BC and reactions
        # must use the physical top face/NSET, not the coordinate maximum.
        top_tol = max(1e-6, 1e-8 * max(abs(float(physical_axis_top.item())), 1.0))
        top_mask_exact = torch.isclose(nodes[:, axis_dim], physical_axis_top, atol=top_tol)
        top_band = max(top_tol, 0.01 * float(bb_tensor[axis_dim].item()))
        top_mask_center = (
            (nodes[:, axis_dim] >= physical_axis_top - top_band)
            & (nodes[:, axis_dim] <= physical_axis_top + top_tol)
        )

        top_mask_nset = _mask_from_node_set('TOP')
        bottom_mask_nset = _mask_from_node_set('BOTTOM')
        left_mask_nset = _mask_from_node_set('LEFT')
        front_mask_nset = _mask_from_node_set('FRONT')

        bottom_mask_exact = bottom_mask_nset if bottom_mask_nset is not None else bottom_mask_coord
        left_mask_exact = left_mask_nset if left_mask_nset is not None else left_mask_coord
        front_mask_exact = front_mask_nset if front_mask_nset is not None else front_mask_coord

        if top_mask_nset is not None:
            top_mask_applied = top_mask_nset
            top_source = 'Abaqus NSET TOP'
        elif bool(top_mask_exact.any()):
            top_mask_applied = top_mask_exact
            top_source = 'exact physical top'
        else:
            top_mask_applied = top_mask_center
            top_source = 'coordinate top band'

        bottom_mask = bottom_mask_exact
        exact_top_count = int(top_mask_exact.sum().item())
        normalized_top_count = int(top_mask_center.sum().item())
        applied_top_count = int(top_mask_applied.sum().item())
        if top_mask_nset is not None or exact_top_count != normalized_top_count:
            logger.info(
                f"[MeshCache] Top-mask coverage: nset={int(top_mask_nset.sum().item()) if top_mask_nset is not None else 0}, "
                f"exact={exact_top_count}, band={normalized_top_count}; "
                f"using {top_source} ({applied_top_count} nodes) for BCs/reactions"
            )
        ...
        norm_coords = (phix, phiy, phiz)
        trans0, trans1 = tuple(getattr(self, 'transverse_dims', (0, 2)))
        if torch.any(bottom_mask):
            bottom_indices = torch.nonzero(bottom_mask, as_tuple=False).squeeze(1)
            bottom_dist = (norm_coords[trans0][bottom_mask] - 0.5) ** 2 + (norm_coords[trans1][bottom_mask] - 0.5) ** 2
            bottom_local_idx = torch.argmin(bottom_dist)
            bottom_center = int(bottom_indices[bottom_local_idx].item())

        top_center = None
        if torch.any(top_mask_applied):
            top_indices = torch.nonzero(top_mask_applied, as_tuple=False).squeeze(1)
            top_dist = (norm_coords[trans0][top_mask_applied] - 0.5) ** 2 + (norm_coords[trans1][top_mask_applied] - 0.5) ** 2
        ...
        cache = {
            ...
            'top_mask_exact': top_mask_exact,
            'bottom_mask_exact': bottom_mask_exact,
            'left_mask_exact': left_mask_exact,
            'front_mask_exact': front_mask_exact,
    ...
    def _construct_displacement(self, mesh_cache, disp_target_val):
        ...
        # ------------------------------------------------------------------
        # Abaqus-consistent displacement construction.
        # Y mode is the legacy plate-with-hole path. Z mode matches the
        # AbaqusVerify cylinder compression deck.
        # ------------------------------------------------------------------
        axis_dim = int(getattr(self, 'load_dim', 1))
        load_axis = getattr(self, 'load_axis', 'y')
        L_axis = float(bounding_box[axis_dim].item() if isinstance(bounding_box, torch.Tensor) else bounding_box[axis_dim])
        L_axis = max(L_axis, 1e-12)
        axial_strain = disp_target / L_axis
        lateral_strain = -self.PR * axial_strain

        U_base = torch.zeros_like(nodes)
        if load_axis == 'z':
            center = getattr(self, 'center_transverse', None)
            if center is None:
                center = torch.stack([nodes[:, 0].mean(), nodes[:, 1].mean()]).to(dtype=torch.float64, device=self.dev)
            U_base[:, 0] = lateral_strain * (nodes[:, 0] - center[0])
            U_base[:, 1] = lateral_strain * (nodes[:, 1] - center[1])
        ...
        # 2) Neural-network correction using normalized coordinates.  For the
        # plate-with-hole case the network is fed local polar features so the
        # stress-concentration perturbation is representable without relying on
        # high-frequency fitting from (x,y,z) alone.
        base_features = torch.stack([phix, phiy, phiz], dim=1)
        input_dim = int(getattr(getattr(self.S_Net, 'linear1', None), 'in_features', base_features.shape[1]))
        if input_dim > 3:
            dx = phix - 0.5
            dy = phiy - 0.5
            r_xy = torch.sqrt(dx ** 2 + dy ** 2 + 1e-24)
            hole_radius = max(float(getattr(self, 'hole_enrich_radius', 0.125)), 1e-12)
            hole_sigma = max(float(getattr(self, 'hole_enrich_sigma', 0.075)), 1e-12)
            hole_band = torch.exp(-((r_xy - hole_radius) / hole_sigma) ** 2)
            cos_theta = dx / torch.clamp(r_xy, min=1e-12)
            sin_theta = dy / torch.clamp(r_xy, min=1e-12)
            inv_decay = hole_radius / (r_xy + hole_radius)
            polar_features = torch.stack([hole_band, cos_theta, sin_theta, inv_decay], dim=1)
            x_in = torch.cat([base_features, polar_features], dim=1)
            if x_in.shape[1] > input_dim:
                x_in = x_in[:, :input_dim]
            elif x_in.shape[1] < input_dim:
                pad = torch.zeros(
                    (x_in.shape[0], input_dim - x_in.shape[1]),
                    dtype=x_in.dtype,
                    device=x_in.device,
                )
            ...
            x_in = base_features
        x_in_net = x_in.to(dtype=self.net_dtype)
        ...
        if num_nodes > batch_size:
            ...
            for i in range(0, num_nodes, batch_size):
                ...
                x_batch = x_in_net[i:end_idx]
                N_batch = self.S_Net(x_batch)
                N_list.append(N_batch.cpu())
                del N_batch, x_batch
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
            N = torch.cat(N_list, dim=0).to(self.dev, dtype=self.physics_dtype)
            del N_list
        else:
            N = self.S_Net(x_in_net).to(dtype=self.physics_dtype)

        # Central-hole softening needs a displacement space that can represent
        # localized strain gradients around the stress concentrator. A smooth
        # global MLP without local enrichment spreads plastic strain over the
        # plate and can select an over-stiff branch even when the return mapping
        # and reaction integration are correct.
        hole_strength = float(getattr(self, 'hole_enrich_strength', 0.0))
        if bool(getattr(self, 'hole_enrich_plastic_only', False)) and not bool(getattr(self, '_hole_enrich_active', False)):
            hole_strength = 0.0
        if load_axis != 'y':
            hole_strength = 0.0
        if hole_strength > 0.0:
            hole_radius = max(float(getattr(self, 'hole_enrich_radius', 0.125)), 1e-12)
            hole_sigma = max(float(getattr(self, 'hole_enrich_sigma', 0.075)), 1e-12)
            r_xy = torch.sqrt((phix - 0.5) ** 2 + (phiy - 0.5) ** 2 + 1e-24)
            hole_band = torch.exp(-((r_xy - hole_radius) / hole_sigma) ** 2)
            N = N * (1.0 + hole_strength * hole_band).unsqueeze(1)

        # Allow the network to express hole-induced displacement perturbations
        # of the same order as the applied displacement.
        scale_mul = float(getattr(self, 'nn_disp_scale', 1.0))
        fixed_scale = torch.clamp(scale_mul * torch.abs(disp_target), min=1e-8)
        ...
        if load_axis == 'z':
            ...
            for center_idx in (getattr(self, 'bottom_center', None), getattr(self, 'top_center', None)):
                if center_idx is not None and 0 <= int(center_idx) < pinning_mask_xy.numel():
                    pinning_mask_xy[int(center_idx)] = 0.0
            ...
            bottom_mask = mesh_cache.get('bottom_mask_exact')
            if bottom_mask is not None and torch.any(bottom_mask):
                U_full[bottom_mask, 2] = 0.0
            if top_mask is not None and torch.any(top_mask):
                U_full[top_mask, 2] = disp_target
            for center_idx in (getattr(self, 'bottom_center', None), getattr(self, 'top_center', None)):
                if center_idx is not None and 0 <= int(center_idx) < U_full.shape[0]:
                    U_full[int(center_idx), 0] = 0.0
                    U_full[int(center_idx), 1] = 0.0
        else:
            hard_factor_x = phix
    ...
    def LE_Gauss(self, u_nodesE, nodesEn, nE, EleConn, Ele_info, eps_p, PEEQ, alpha,
        ...
        if self.EXAMPLE in [4, 5]:
            # Optional regularizer only. For non-associated DP softening this is
            # not a variational potential; using it with O(1) weight biases the
            # global solve toward too little PEEQ and an over-stiff force curve.
            # The default Softening setup therefore leaves it off.
            dissipation_weight = float(getattr(
                self,
                '_softening_dissipation_active_weight',
                getattr(self, 'softening_dissipation_weight', 0.0)
            ))
            if dissipation_weight != 0.0:
                diss_new = dp_softening_dissipation(PEEQ_new)
                diss_old = dp_softening_dissipation(PEEQ.detach() if isinstance(PEEQ, torch.Tensor) else PEEQ)
                diss_increment = torch.clamp(diss_new - diss_old, min=0.0)
                energy_density = energy_el_density + dissipation_weight * diss_increment
            else:
    ...
    def get_physical_internal_energy(self, u_nodesE, eps_p, PEEQ, alpha):
        ...
        if self.EXAMPLE in [4, 5]:
            ...
            if dissipation_weight != 0.0:
                energy_density = energy_density + dissipation_weight * dp_softening_dissipation(PEEQ_const)
    ...
    def reaction_via_energy(self, nodesEn, eps_p, PEEQ, alpha, applied_disp):
        """Return top reaction as -dΠ/da, parameterizing a only on the loaded top DOF.

        This enforces dU/da = e_axis on top nodes and 0 elsewhere, guaranteeing that
        the energy-derived reaction matches the assembled/face reaction at the same state.
        """
        try:
        ...
        # Build parametric field: only the physical top-surface loaded DOF depends on a.
        # Do not use coordinate max directly, because Abaqus decks can include a
        # reference node outside the physical domain.
        top_mask = self._get_top_surface_node_mask(nodesEn)
        ...
        load_dim = int(getattr(self, 'load_dim', 1))
        # Add (a - a0) only to the loaded top displacement component.
        ...
        U[:, load_dim] = U[:, load_dim] + (a - a0) * top_mask_f
    ...
    def _run_dp_gradcheck(self):
        ...
        # Compute H_d for this PEEQ from the shared softening table
        H_d = self.HardeningModulus(P0)
        if not isinstance(H_d, torch.Tensor):
            H_d = torch.tensor(H_d, dtype=torch.float64, device=self.dev)
        ...
        def f(strain_input):
            sigma, eps_p_new, peeq_new = DruckerPragerPlasticity.apply(
                strain_input, eps_p_old, P0, D_tensor, G, K, tanB, tanPsi, d0, H_d, True
            )
    ...
    def _assembled_equilibrium_residual(self, stress_gp, Ele_info, EleConn):
        ...
        # Dirichlet DOFs for assembled equilibrium residual must match the hard
        # displacement construction exactly. Otherwise legitimate support
        # reactions are penalized as free-DOF residuals and the optimizer can
        # drift toward an artificially low-reaction branch.
        mesh_cache = getattr(self, '_primary_mesh_cache', None)
        ...
        if mesh_cache is not None:
            bottom_nodes = mesh_cache.get('bottom_mask_exact')
            top_nodes = mesh_cache.get('top_mask')
            left_nodes = mesh_cache.get('left_mask_exact')
            front_nodes = mesh_cache.get('front_mask_exact')
        else:
            x_coords = nodes[:, 0]
            y_coords = nodes[:, 1]
            z_coords = nodes[:, 2]
            axis_dim = int(getattr(self, 'load_dim', 1))
            axis_coords = nodes[:, axis_dim]
            ...
            tol_x = torch.clamp(torch.max(x_coords) - x_min, min=1e-12) * 1e-6
            tol_axis = torch.clamp(axis_max - axis_min, min=1e-12) * 1e-6
            tol_z = torch.clamp(torch.max(z_coords) - z_min, min=1e-12) * 1e-6
            bottom_nodes = axis_coords <= (axis_min + tol_axis)
            top_nodes = axis_coords >= (axis_max - tol_axis)
            left_nodes = x_coords <= (x_min + tol_x)
            front_nodes = z_coords <= (z_min + tol_z)

        load_dim = int(getattr(self, 'load_dim', 1))
        load_axis = getattr(self, 'load_axis', 'y')
        dirichlet_mask[bottom_nodes, load_dim] = True
        dirichlet_mask[top_nodes, load_dim] = True
        if load_axis == 'z':
            for center_idx in (getattr(self, 'bottom_center', None), getattr(self, 'top_center', None)):
                if center_idx is not None and 0 <= int(center_idx) < dirichlet_mask.shape[0]:
                    dirichlet_mask[int(center_idx), 0] = True
                    dirichlet_mask[int(center_idx), 1] = True
        else:
            dirichlet_mask[left_nodes, 0] = True
            dirichlet_mask[front_nodes, 2] = True

        if not hasattr(self, '_eq_dirichlet_logged'):
            self._eq_dirichlet_logged = True
            try:
                if load_axis == 'z':
                    logger.info(
                        "[Equilibrium Residual] Dirichlet DOFs excluded from residual: "
                        f"bottom Uz={int(bottom_nodes.sum().item())}, "
                        f"top Uz={int(top_nodes.sum().item())}, "
                        "bottom/top center Ux,Uy pins"
                    )
                else:
                    logger.info(
                        "[Equilibrium Residual] Dirichlet DOFs excluded from residual: "
                        f"bottom Uy={int(bottom_nodes.sum().item())}, "
                        f"top Uy={int(top_nodes.sum().item())}, "
                        f"left Ux={int(left_nodes.sum().item())}, "
                        f"front Uz={int(front_nodes.sum().item())}"
                    )
            except Exception:
                pass
        ...
        free_values = torch.abs(F_free[free_mask])
        residual = torch.linalg.norm(free_values, ord=2)

        free_dof_count = free_values.numel()
        if free_dof_count > 0:
            residual = residual / torch.sqrt(torch.as_tensor(free_dof_count, dtype=torch.float64, device=self.dev))

        # Optional diagnostic variant: add a high-residual top-k term. Keep this
        # disabled by default; in the softening benchmark it can dominate the
        # variational objective and move the solution to an over-stiff branch.
        topk_fraction = float(getattr(self, 'equilibrium_topk_fraction', 0.0))
        topk_weight = float(getattr(self, 'equilibrium_topk_weight', 0.0))
        if topk_fraction > 0.0 and topk_weight > 0.0 and free_values.numel() > 0:
            k_top = max(1, int(math.ceil(topk_fraction * free_values.numel())))
            k_top = min(k_top, int(free_values.numel()))
            top_values = torch.topk(free_values, k=k_top, largest=True, sorted=False).values
            local_residual = torch.linalg.norm(top_values, ord=2) / torch.sqrt(
                torch.as_tensor(k_top, dtype=torch.float64, device=self.dev)
            )
            residual = residual + topk_weight * local_residual
    ...
    def loss_function(self, u_nodesE, step, epoch, nodesEn, applied_u, eps_p, PEEQ, alpha, Ele_info, EleConn, nE):
        """
        以有限元装配的虚功残量作为塑性/软化阶段主约束。

        - 储能项：`LE_Gauss` 返回的总内能；非关联软化阶段默认关闭。
        - 虚功项：将高斯点应力装配成节点内力，对非位移控制自由度求残量平方。

        非关联 Drucker-Prager 没有全局标量势；塑性/软化阶段不能让
        储能项替代虚功平衡。软化后如果让储能项参与分支选择，
        可得到屈服面干净但 PEEQ 演化不足、反力偏高的平衡分支。
        """
        ...
        self._dp_training_mode = True
        try:
            internal_energy, stress_gp, _, PEEQ_new_loss, _ = self.LE_Gauss(
                u_nodesE,
                nodesEn,
                nE,
                EleConn,
        ...
        internal_weight_value = float(getattr(self, '_internal_energy_active_weight', 1.0))
        internal_weight = torch.tensor(internal_weight_value, dtype=torch.float64, device=self.dev)
        internal_loss = internal_weight * internal_energy
        ...
        if eq_weight_value > 0.0:
            ...
            eq_loss = eq_weight * eq_residual
            total_loss = total_loss + eq_loss

        peeq_trust_weight_value = float(getattr(self, '_peeq_increment_trust_active_weight', 0.0))
        peeq_trust_residual = None
        peeq_trust_loss = None
        peeq_trust_upper_residual = None
        peeq_trust_lower_residual = None
        if peeq_trust_weight_value > 0.0:
            cap_value = float(getattr(self, '_peeq_increment_trust_cap', float('inf')))
            if math.isfinite(cap_value) and cap_value > 0.0:
                cap_tensor = torch.tensor(cap_value, dtype=torch.float64, device=self.dev)
                dpeeq_loss = torch.clamp(PEEQ_new_loss - PEEQ, min=0.0)
                excess = torch.relu(dpeeq_loss - cap_tensor)
                peeq_trust_upper_residual = torch.mean((excess / cap_tensor) ** 2)
                peeq_trust_residual = peeq_trust_upper_residual

                trust_target = getattr(self, '_peeq_increment_trust_target', None)
                trust_mask = getattr(self, '_peeq_increment_trust_mask', None)
                trust_points = int(getattr(self, '_peeq_increment_trust_target_points', 0))
                if trust_points > 0 and isinstance(trust_target, torch.Tensor) and isinstance(trust_mask, torch.Tensor):
                    dpeeq_flat = dpeeq_loss.reshape(-1)
                    target_flat = trust_target.to(
                        device=self.dev,
                        dtype=torch.float64
                    ).reshape(-1)
                    mask_flat = trust_mask.to(device=self.dev).reshape(-1).bool()
                    if dpeeq_flat.numel() == target_flat.numel() == mask_flat.numel():
                        lower_factor = float(getattr(self, 'peeq_increment_trust_lower_factor', 0.25))
                        lower_factor = max(0.0, min(lower_factor, 1.0))
                        target_sel = target_flat[mask_flat]
                        dpeeq_sel = dpeeq_flat[mask_flat]
                        lower = lower_factor * target_sel
                        denom = torch.clamp(target_sel, min=1e-8)
                        deficit = torch.relu(lower - dpeeq_sel)
                        peeq_trust_lower_residual = torch.mean((deficit / denom) ** 2)
                        peeq_trust_residual = peeq_trust_residual + peeq_trust_lower_residual

                peeq_trust_loss = torch.tensor(
                    peeq_trust_weight_value, dtype=torch.float64, device=self.dev
                ) * peeq_trust_residual
                total_loss = total_loss + peeq_trust_loss

        if epoch in (0, 2000, 4000):
            if not hasattr(self, '_loss_component_logged'):
                self._loss_component_logged = set()
            loss_key = (int(step), int(epoch))
            if loss_key not in self._loss_component_logged:
                self._loss_component_logged.add(loss_key)
                try:
                    eq_residual_val = float(eq_residual.detach().item()) if isinstance(eq_residual, torch.Tensor) else 0.0
                    eq_loss_val = float(eq_loss.detach().item()) if isinstance(eq_loss, torch.Tensor) else 0.0
                    peeq_trust_val = float(peeq_trust_residual.detach().item()) if isinstance(peeq_trust_residual, torch.Tensor) else 0.0
                    peeq_trust_upper_val = float(peeq_trust_upper_residual.detach().item()) if isinstance(peeq_trust_upper_residual, torch.Tensor) else 0.0
                    peeq_trust_lower_val = float(peeq_trust_lower_residual.detach().item()) if isinstance(peeq_trust_lower_residual, torch.Tensor) else 0.0
                    peeq_trust_loss_val = float(peeq_trust_loss.detach().item()) if isinstance(peeq_trust_loss, torch.Tensor) else 0.0
                    logger.info(
                        f"[Loss components] Step {step} Epoch {epoch}: "
                        f"internal={float(internal_energy.detach().item()):.6e}, "
                        f"internal_weight={internal_weight_value:.3e}, "
                        f"internal_loss={float(internal_loss.detach().item()):.6e}, "
                        f"eq_weight={eq_weight_value:.3e}, eq_residual={eq_residual_val:.6e}, "
                        f"eq_loss={eq_loss_val:.6e}, peeq_trust_weight={peeq_trust_weight_value:.3e}, "
                        f"peeq_trust_residual={peeq_trust_val:.6e}, "
                        f"peeq_trust_upper={peeq_trust_upper_val:.6e}, "
                        f"peeq_trust_lower={peeq_trust_lower_val:.6e}, "
                        f"peeq_trust_loss={peeq_trust_loss_val:.6e}, "
                        f"total={float(total_loss.detach().item()):.6e}"
                    )
                except Exception:
                    pass
    ...
    def _default_softening_force_reference(self):
        """Abaqus force-displacement curve used as a path sanity check for the Z-compression softening cylinder."""
        disp = np.array([
            0.0, 0.001, 0.002, 0.0035, 0.00575, 0.009125, 0.01
        ], dtype=np.float64)
        force = np.array([
            0.0, 18.8705, 18.1709, 17.1221, 15.5896, 14.8778, 14.6974
        ], dtype=np.float64)
        return disp, force, 'built-in Abaqus cylinder table'

    def _read_softening_force_reference_csv(self, csv_path):
        """Read a two-column displacement/reaction reference CSV."""
        disp_keys = ('top_disp', 'top_disp_uni', 'DisplacementAbaqus', 'displacement', 'disp', 'u')
        force_keys = ('top_reaction_force', 'ReactionForceAbaqus', 'reaction_force', 'force', 'rf')
        disp_vals = []
        force_vals = []
        ...
        with open(csv_path, 'r', newline='') as fcsv:
            ...
            for key in disp_keys:
                if key.lower() in field_lookup:
                    disp_col = field_lookup[key.lower()]
                    break
            for key in force_keys:
                if key.lower() in field_lookup:
                    force_col = field_lookup[key.lower()]
                    break
            if disp_col is None or force_col is None:
                if len(fields) < 2:
                    raise ValueError(f'reference CSV needs two numeric columns, got {fields}')
                disp_col, force_col = fields[0], fields[1]

            for row in reader:
                try:
                    disp_vals.append(abs(float(row[disp_col])))
                    force_vals.append(abs(float(row[force_col])))
                except (TypeError, ValueError, KeyError):
                    continue

        if len(disp_vals) < 2:
            raise ValueError(f'reference CSV has only {len(disp_vals)} valid points')

        order = np.argsort(np.asarray(disp_vals, dtype=np.float64))
        disp = np.asarray(disp_vals, dtype=np.float64)[order]
        force = np.asarray(force_vals, dtype=np.float64)[order]
        return disp, force, csv_path

    def _get_softening_force_reference(self):
        if self._softening_force_reference_cache is not None:
            return self._softening_force_reference_cache

        if self.softening_force_reference_csv:
            try:
                self._softening_force_reference_cache = self._read_softening_force_reference_csv(
                    self.softening_force_reference_csv
                )
                return self._softening_force_reference_cache
            except Exception as exc:
                logger.warning(
                    f"[SOFTENING-PATH] Failed to read DP_ABAQUS_FORCE_CSV="
                    f"{self.softening_force_reference_csv}: {exc}. Falling back to built-in Abaqus table."
                )

        self._softening_force_reference_cache = self._default_softening_force_reference()
        return self._softening_force_reference_cache

    def _query_softening_force_reference(self, disp_abs):
        disp_ref, force_ref, source = self._get_softening_force_reference()
        disp_abs = abs(float(disp_abs))
        if disp_abs < float(disp_ref[0]) or disp_abs > float(disp_ref[-1]):
            return None, source
        return float(np.interp(disp_abs, disp_ref, force_ref)), source

    def _record_softening_path_diagnostics(
        self,
        step,
        disp_abs,
        force_abs,
        ...
        """Persist path-level checks that local yield residual tests cannot see."""
        if int(self.EXAMPLE) != 5:
            return

        disp_abs = abs(float(disp_abs))
        force_abs = abs(float(force_abs))
        peeq_new_flat = peeq_new.detach().reshape(-1).double()
        peeq_new_max = float(peeq_new_flat.max().item()) if peeq_new_flat.numel() else 0.0
        peeq_old_max = float('nan')
        dpeeq_max = float('nan')
        dpeeq_mean_pos = float('nan')

        if peeq_old is not None:
            peeq_old_flat = peeq_old.detach().reshape(-1).to(
                device=peeq_new_flat.device,
                dtype=peeq_new_flat.dtype
            )
            if peeq_old_flat.numel() == peeq_new_flat.numel():
                peeq_old_max = float(peeq_old_flat.max().item()) if peeq_old_flat.numel() else 0.0
                dpeeq = torch.clamp(peeq_new_flat - peeq_old_flat, min=0.0)
                dpeeq_max = float(dpeeq.max().item()) if dpeeq.numel() else 0.0
                positive = dpeeq[dpeeq > 0.0]
                dpeeq_mean_pos = float(positive.mean().item()) if positive.numel() else 0.0

        ref_force, ref_source = self._query_softening_force_reference(disp_abs)
        force_rel_error = float('nan')
        if ref_force is not None:
            force_rel_error = (force_abs - ref_force) / max(abs(ref_force), 1e-12)

        prev = self._softening_path_prev
        force_jump_abs = float('nan')
        force_jump_rel = float('nan')
        if prev is not None:
            force_jump_abs = force_abs - float(prev.get('force_abs', 0.0))
            force_jump_rel = force_jump_abs / max(abs(float(prev.get('force_abs', 0.0))), 1e-12)

        reasons = []
        ref_bad = (
            ref_force is not None
            and int(step) >= self.softening_failfast_min_step
            and abs(force_rel_error) >= self.softening_failfast_ref_rel_tol
        )
        if ref_bad:
            reasons.append(
                f"reference_error={force_rel_error:.3%} exceeds {self.softening_failfast_ref_rel_tol:.3%}"
            )

        frozen_branch = (
            prev is not None
            and peeq_old_max > 1e-5
            and dpeeq_max <= self.softening_failfast_frozen_dpeeq_tol
            and force_jump_abs >= self.softening_failfast_force_jump_abs
            and force_jump_rel >= self.softening_failfast_force_jump_rel
        )
        if frozen_branch:
            reasons.append(
                f"force jumped {force_jump_abs:.3e} N while max dPEEQ={dpeeq_max:.3e}"
            )

        out_csv = os.path.join(self.base, 'softening_path_diagnostics.csv')
        ...
        with open(out_csv, 'a', newline='') as fcsv:
            ...
            if write_header:
                writer.writerow([
                    'step', 'top_disp_abs', 'force_dem_abs', 'force_ref',
                    'force_rel_error', 'reference_source', 'peeq_old_max',
                    'peeq_new_max', 'dpeeq_max', 'dpeeq_mean_pos',
                    'peeq_frac_gt_1e-5', 'peeq_frac_gt_1e-4',
                    'peeq_frac_gt_1e-3', 'peeq_frac_gt_0.005',
                    'd_min', 'd_max', 'force_jump_abs', 'force_jump_rel',
                    'wrong_branch_reason', 'failfast'
                ])
            writer.writerow([
                int(step), disp_abs, force_abs,
                (ref_force if ref_force is not None else ''),
                force_rel_error, ref_source, peeq_old_max,
                peeq_new_max, dpeeq_max, dpeeq_mean_pos,
                frac_1e5, frac_1e4, frac_1e3, frac_first,
                d_min, d_max, force_jump_abs, force_jump_rel,
                '; '.join(reasons),
                int(self.softening_failfast_enabled and bool(reasons))
            ])

        ref_log = f"{ref_force:.6e}" if ref_force is not None else "N/A"
        logger.info(
            f"[SOFTENING-PATH] Step {step}: disp={disp_abs:.6e}, "
            f"F_DEM={force_abs:.6e} N, F_ref={ref_log} N, "
            f"rel_err={force_rel_error:.3%}, PEEQ_old_max={peeq_old_max:.3e}, "
            f"PEEQ_new_max={peeq_new_max:.3e}, dPEEQ_max={dpeeq_max:.3e}"
        )

        if self.softening_failfast_enabled and reasons:
            self._softening_failfast_pending = (
                f"[SOFTENING-FAILFAST] Step {step}: invalid softening load path detected: "
                f"{'; '.join(reasons)}. Diagnostics written to {out_csv}"
            )
            logger.error(self._softening_failfast_pending)
        elif reasons:
            logger.warning(
                f"[SOFTENING-PATH-WARNING] Step {step}: invalid softening load path detected: "
                f"{'; '.join(reasons)}. Continuing because DP_SOFTENING_FAILFAST=0. "
                f"Diagnostics written to {out_csv}"
            )

        self._softening_path_prev = {
            'step': int(step),
            'disp_abs': disp_abs,
            'force_abs': force_abs,
            'peeq_new_max': peeq_new_max,
        }
    ...
    def SaveData( self , domain , U , ip_out , final_state, LBFGS_loss , step , ref_file ):
        ...
        # Verify yield admissibility after return mapping.
        # PEEQ>0 is a history flag, not an active-yield flag: a previously plastic
        # point may unload into f<0. Therefore the robust global check is max(f+)<=tol.
        try:
            d_eff_diag = self._effective_cohesion(PEEQ_data)
            f_diag, p_diag, q_diag = dp_yield_function(stress_gp_final, self.TAN_BETA, d_eff_diag)
            max_f_positive = float(torch.clamp(f_diag, min=0.0).max().item())
            max_peeq_check_local = float(PEEQ_data.max().item())
            history_mask = PEEQ_data > 1e-12
            if bool(history_mask.any()):
                f_history = f_diag[history_mask]
                max_abs_history_f = float(torch.abs(f_history).max().item())
                min_history_f = float(f_history.min().item())
                max_history_f = float(f_history.max().item())
            else:
                max_abs_history_f = 0.0
                min_history_f = 0.0
                max_history_f = 0.0
            
            if max_f_positive > 1e-8:
                # DEPTHCORE-FIX: Contextual severity based on magnitude
                # Large violations (>σ_yield) indicate algorithmic failure
                # Positive f means the stress lies outside the yield surface.
                sigma_c_val = float(getattr(self, 'sigma_c_yield', 24.35))
                if max_f_positive > sigma_c_val:
                    logger.error(
                        f"[SaveData Step {step}] CRITICAL: Yield violation exceeds σ_c! "
                        f"max(f+) = {max_f_positive:.3e} > σ_c={sigma_c_val:.2f}. "
                        f"Return mapping failed - check PEEQ_max={max_peeq_check_local:.3e}"
                    )
                elif max_f_positive > 10.0:
                    logger.warning(
                        f"[SaveData Step {step}] Yield surface violation on TRAINING MESH: "
                        f"max(f+) = {max_f_positive:.3e} > 1e-8. This affects physics! "
                        f"(Newton iterations may need tuning, or first plastic step lag)"
                    )
                else:
                    logger.info(
                        f"[SaveData Step {step}] Minor yield residual on training mesh: "
                        f"max(f+) = {max_f_positive:.3e} > 1e-8"
                    )
            elif max_abs_history_f > 1e-8 and max_peeq_check_local > 1e-5:
                logger.info(
                    f"[SaveData Step {step}] Yield admissibility OK: max(f+)={max_f_positive:.3e}. "
                    f"History-plastic points include inactive states: max|f|={max_abs_history_f:.3e}, "
                    f"f_range=[{min_history_f:.3e}, {max_history_f:.3e}]"
                )
        except Exception as e:
        ...
        # Top-surface reference area used for coverage checks and stress normalization.
        # For this plate-with-hole setup, the relevant footprint is the x-z plane extent.
        nominal_area = None
        try:
            bb = domain.get('BB', None) if isinstance(domain, dict) else None
            if bb is not None and len(bb) >= 3:
                bb_vals = [
                    float(v.item()) if isinstance(v, torch.Tensor) else float(v)
                    for v in bb[:3]
                ]
                nominal_area = max(bb_vals[0] * bb_vals[2], 1e-12)
        except Exception as e_nominal:
            logger.debug(f"[SaveData Step {step}] Could not derive nominal top area from BB: {e_nominal}")
        if nominal_area is None:
            nominal_area = float(getattr(self, '_top_face_reference_area', 1.0))
            nominal_area = max(nominal_area, 1e-12)
        ...
        # Abaqus reference/control nodes may sit outside the physical mesh (e.g. z=1.1
        # for a z=1 cylinder). They are not connected to elements, so their displacement
        # should be the prescribed reference displacement, not phiz * disp_target.
        U_reported = map_sign(U, 'strain')
        U_raw_vtk = U.clone()
        U_reported_vtk = U_reported.clone()
        try:
            used_node_mask = torch.zeros(U.shape[0], dtype=torch.bool, device=U.device)
            conn_flat = self.domain['EleConn'].reshape(-1).to(device=U.device, dtype=torch.long)
            valid_conn = conn_flat[(conn_flat >= 0) & (conn_flat < U.shape[0])]
            used_node_mask[valid_conn] = True
            ref_node_mask = ~used_node_mask
            if torch.any(ref_node_mask):
                load_dim_vtk = int(getattr(self, 'load_dim', 1))
                disp_raw_vtk = torch.as_tensor(self.applied_disp, dtype=U.dtype, device=U.device)
                disp_report_vtk = map_sign(disp_raw_vtk, 'strain')
                U_raw_vtk[ref_node_mask, :] = 0.0
                U_reported_vtk[ref_node_mask, :] = 0.0
                U_raw_vtk[ref_node_mask, load_dim_vtk] = disp_raw_vtk
                U_reported_vtk[ref_node_mask, load_dim_vtk] = disp_report_vtk
        except Exception as vtk_ref_node_err:
            logger.debug(f"[VTK] Could not normalize reference-node displacement: {vtk_ref_node_err}")
        ...
        def _transfer_vtk_arrays():
            ...
            _vtk_transfer_payload['U_np'] = U_reported_vtk.detach().cpu().numpy()
            _vtk_transfer_payload['U_raw_np'] = U_raw_vtk.detach().cpu().numpy()
        ...
        U_np_raw = _vtk_transfer_payload['U_raw_np']

        # Add nodal displacement data.  Abaqus/ParaView commonly expects a vector
        # named "U"; keep scalar components for backward-compatible plotting scripts.
        grid.point_data['U'] = U_np_reported
        grid.point_data['U_raw'] = U_np_raw
        try:
        ...
        try:
            # Use the cached top-surface node mask when available; fall back to exact y-max matching.
            top_mask = self._get_top_surface_node_mask(nodesEn)
            if not torch.any(top_mask):
                logger.error("Top surface nodes not found from cached mesh metadata")
            ...
            load_dim = int(getattr(self, 'load_dim', 1))
            load_axis_label = getattr(self, 'load_axis_label', 'Y')
            stress_axis_label = getattr(self, 'stress_axis_label', f"S{load_dim + 1}{load_dim + 1}")
            top_u = U_tensor[top_mask, load_dim].mean()  # Mean loaded displacement of top surface
            # Sanity check: top displacement should equal applied displacement (within tol)
            try:
                disp_tol = 1e-9
                if abs(top_u.item() - float(self.applied_disp)) > max(disp_tol, 1e-9 * max(1.0, abs(float(self.applied_disp)))):
                    logger.warning(f"Top U{load_axis_label.lower()} ({top_u.item():.6e}) != applied_disp ({float(self.applied_disp):.6e}); boundary kinematics may be drifting")
            except Exception:
                pass
            logger.debug(f"Raw top_u{load_axis_label.lower()} (before map_sign): {top_u.item():.6e}")
            ...
            try:
                ...
                surface_order = int(getattr(self, 'face_integration_order', 2))
                ...
                for face_idx, (face_nodes, _fixed_axis, _fixed_value) in enumerate(face_defs):
                    candidate_mask = torch.ones(EleConn_local.shape[0], dtype=torch.bool, device=self.dev)
                    for local_node_idx in face_nodes:
                        candidate_mask &= top_node_mask[EleConn_local[:, local_node_idx]]

                    eligible_idx = torch.nonzero(candidate_mask, as_tuple=False).squeeze(-1)
                    if eligible_idx.numel() == 0:
                        continue

                    eligible_face_count += int(eligible_idx.numel())
                    coords_e_all = nodesEn[EleConn_local[eligible_idx], :]  # [k,8,3]
                    stress_gp_subset = stress_gp[:, eligible_idx]  # [8, k, 3, 3]
                    face_area_total = 0.0

                    for xi, eta, zeta, weight_face in self._hex8_face_quadrature(face_idx, surface_order=surface_order):
                        B_nat = self._hex_shape_gradients(xi, eta, zeta)  # [8,3]
                        ...
                        if face_idx in (0, 1):
                            t1, t2 = dX_dxi, dX_deta
                        elif face_idx in (2, 4):
                            t1, t2 = dX_dxi, dX_dzeta
                        else:
                            t1, t2 = dX_deta, dX_dzeta

                        n_vec = torch.cross(t1, t2, dim=1)
                        area_weight = torch.norm(n_vec, dim=1)
                        ...
                        flip_mask = (n_vec_v[:, load_dim] < 0.0).unsqueeze(1)
                        ...
                        N_face = self._hex_shape_functions(xi, eta, zeta)  # [8]
                        stress_face_all = torch.sum(N_face.view(8, 1, 1, 1) * stress_gp_subset, dim=0)
                        stress_face = stress_face_all[valid]

                        traction_face = torch.einsum('bij,bj->bi', stress_face, n_hat_v)
                        reaction_face_vec = reaction_face_vec - (traction_face * area_weighted.unsqueeze(1)).sum(dim=0)
                        s_axis_area_sum += float((stress_face[:, load_dim, load_dim] * area_weighted).sum().item())

                    face_area_integrated += face_area_total

                if eligible_face_count > 0 and face_area_integrated > 0.0:
                    reaction_face = reaction_face_vec[load_dim]
                    s_axis_face_mean = s_axis_area_sum / face_area_integrated
                    coverage_raw = face_area_integrated / max(nominal_area, 1e-12)
                    if coverage_raw >= 0.5:
                        self._top_face_reference_area = face_area_integrated
                        area_reference = face_area_integrated
                    area_reference_safe = max(area_reference, 1e-12)
                    area_ratio = face_area_integrated / area_reference_safe

                    logger.info(
                        f"[SaveData] Top face quadrature: faces={eligible_face_count}, "
                        f"A_face_int={face_area_integrated:.6e}, A_target={area_reference_safe:.6e}, "
                        f"coverage={area_ratio:.3%}"
                    )

                    if area_ratio < 0.98:
                        logger.warning(f"[SaveData] Area coverage is {area_ratio:.1%} - may be missing top faces. Expected ~100%.")
                    ...
                    logger.warning("[SaveData] No top faces detected from HEX8 face matching; falling back to assembled internal forces.")
                    logger.warning(f"[SaveData] A_face_int={face_area_integrated:.6e} (top-node mask size={int(torch.sum(top_node_mask).item())})")
            ...
            reaction_asm = -Fint_nodes[top_mask, load_dim].sum()  # external reaction
            ...
            disp_val = float(top_u.item())  # Raw value, no map_sign yet
            disp_val_reported = float(map_sign(torch.tensor(disp_val, device=self.dev), 'strain').item())

            if abs(disp_val) > 1e-15:
                force_report_sign = -1.0 if disp_val > 0.0 else 1.0
            else:
                force_report_sign = 1.0

            def _report_reaction_force(raw_force):
                return force_report_sign * float(raw_force)
            ...
            # Prefer top-face area-averaged axial stress for traction-driven comparison; fallback to volume mean
            if (reaction_face is not None) and (face_area_integrated > 0.0):
                s33_vol_mean_val_raw = float(s_axis_face_mean)
            else:
                s33_vol_mean_raw = (stress_gp[..., load_dim, load_dim] * weight_gp).sum() / total_w_safe
            ...
            if self.guardrail_verbose:
                logger.debug(f"[SaveData] {stress_axis_label}_volmean={s33_vol_mean_val:.6e}, A*{stress_axis_label}={A_sigma:.6e}")

            # Report reactions as positive load magnitudes along the prescribed loading path.
            reaction_force_val = _report_reaction_force(reaction_val)
            ...
            selected_force_val = _report_reaction_force(reaction_selected)
            s33_from_reaction_val = selected_force_val / area_for_s33_value
            top_mask_count = int(torch.sum(top_mask).item()) if hasattr(top_mask, 'sum') else 0
            logger.info(
                f"[SaveData] Top surface displacement (unified, m): {disp_val_reported:.6e}, "
                f"Reaction_base (N, {method_used}): {reaction_force_val:.6e}, "
                f"Reaction_sel (N, {method_selected}): {selected_force_val:.6e}, "
                f"{stress_axis_label}_from_reaction (MPa): {s33_from_reaction_val:.6e}"
            )

            face_force_val = _report_reaction_force(reaction_face.item()) if reaction_face is not None else None
            asm_force_val = _report_reaction_force(reaction_asm.item())
            ...
            logger.info(
                ...
                f"F_face={face_force_log}, F_asm={asm_force_val:.6e}, A*{stress_axis_label}={A_sigma:.6e}"
            )

            # Persist the CSV early so later diagnostics cannot suppress the history file.
            reaction_val_reported = reaction_force_val
            s33_from_reaction_reported = s33_from_reaction_val
            s33_vol_mean_reported = s33_vol_mean_val
            out_csv = os.path.join(self.base, 'top_surface_reaction_displacement.csv')
            os.makedirs(self.base, exist_ok=True)
            write_header = (not os.path.exists(out_csv)) or (os.path.getsize(out_csv) == 0)
            try:
                with open(out_csv, 'a', newline='') as fcsv:
                    w = csv.writer(fcsv)
                    if write_header:
                        # top_reaction_force stores the current-step canonical top reaction; method disambiguates the source.
                        if load_dim == 2:
                            w.writerow(['step', 'top_disp', 'top_disp_uni', 's33_volmean', 'top_s33_uni', 'top_reaction_force', 'top_reaction_method', 's33_from_reaction', 'A_top_face', 'top_reaction_energy', 'top_reaction_face', 'top_reaction_asm'])
                        else:
                            w.writerow(['step', 'top_disp', 'top_disp_uni', 's22_volmean', 'top_s33_uni', 'top_reaction_force', 'top_reaction_method', 's22_from_reaction', 'A_top_face', 'top_reaction_energy', 'top_reaction_face', 'top_reaction_asm'])
                    area_used = area_for_s33_value
                    # Use reported values with consistent sign convention (applied at final step only).
                    # top_reaction_force remains the canonical reaction selected for this step.
                    w.writerow([
                        step,
                        disp_val_reported,
                        disp_val_reported,
                ...
                logger.warning(f'[SaveData] WARNING: failed to write top surface reaction-displacement data: {e_csv}')
            ...
            if self.enable_energy_gradcheck and allow_energy_method and (step % 10 == 0):
                try:
            ...
            if s33_from_reaction_val * reaction_val < 0:
                logger.warning(f"[SaveData] Compression-positive convention mismatch: reaction force and {stress_axis_label} have opposite signs")
            if s33_vol_mean_val * reaction_val < 0:
                logger.warning(f"[SaveData] Compression-positive convention mismatch: volume-mean {stress_axis_label} disagrees with reaction force sign")
            ...
            try:
                ...
                peeq_flat_diag = PEEQ_data.reshape(-1)
                first_softening_strain = 5.0e-3
                frac_1e5 = float((peeq_flat_diag > 1e-5).double().mean().item())
                frac_1e4 = float((peeq_flat_diag > 1e-4).double().mean().item())
                frac_1e3 = float((peeq_flat_diag > 1e-3).double().mean().item())
                frac_first = float((peeq_flat_diag > first_softening_strain).double().mean().item())
                logger.info(
                    f"[SaveData] Softening activation: max_PEEQ/0.005={peeq_gp_max / first_softening_strain:.3%}, "
                    f"vol_frac(PEEQ>1e-5)={frac_1e5:.3%}, "
                    f">1e-4={frac_1e4:.3%}, >1e-3={frac_1e3:.3%}, "
                    f">0.005={frac_first:.3%}"
                )
                d_eff_report = self._effective_cohesion(PEEQ_data)
                d_min_report = float(d_eff_report.min().item())
                d_max_report = float(d_eff_report.max().item())
                if peeq_gp_max > 1e-5:
                    logger.info(
                        f"[SaveData] Softening d(PEEQ) range: "
                        f"min={d_min_report:.6e}, "
                        f"max={d_max_report:.6e}"
                    )
                self._record_softening_path_diagnostics(
                    step=step,
                    disp_abs=disp_val_reported,
                    force_abs=selected_force_val,
                    peeq_old=getattr(self, '_step_peeq_initial_for_diagnostics', None),
                    peeq_new=PEEQ_data,
                    d_min=d_min_report,
                    d_max=d_max_report,
                    frac_1e5=frac_1e5,
                ...
                logger.debug(f"[SOFTENING-PATH] Could not write path diagnostics: {exc}")
            ...
            if rel_diff > 0.045:  # 4.5% tolerance (equilibrium residual)
                try:
                    s33_elem_avg = float(mean_stress[load_dim]) if isinstance(mean_stress, np.ndarray) else float(mean_stress[load_dim])
                except Exception:
                    s33_elem_avg = float('nan')
                logger.warning(
                    f"[SaveData] Reaction vs A*{stress_axis_label} mismatch >4.5%: |F_top|={abs(selected_force_val):.6e}, "
                    f"method={method_selected}, top_mask={top_mask_count}, "
                    f"raw_face={float(reaction_face.item()) if reaction_face is not None else float('nan'):.6e}, "
                    f"raw_asm={float(reaction_asm.item()):.6e}, area_ratio={area_ratio:.3f}, "
                    f"|A*{stress_axis_label}_volmean|={abs(A_sigma):.6e}, rel={rel_diff:.3%}, {stress_axis_label}_elemAvg={s33_elem_avg:.6e}"
                )
            # Validation check for rigid body constraint effectiveness
            bottom_u_violation = abs(U_tensor[self.bottom_center, load_dim].item())
            if bottom_u_violation > 1e-4:
                logger.warning(f"Rigid body constraint violation: bottom center U{load_axis_label.lower()} = {bottom_u_violation:.6e}. Consider increasing rigid_penalty_weight.")
            ...
            try:
                ...
                if max_peeq > YIELD_ONSET_THRESHOLD:
                    ...
                    if hasattr(self, 'TAN_BETA') and hasattr(self, 'cohesion_d'):
                        d_eff_diag = self._effective_cohesion(PEEQ_data)
                        f_gp, p_gp, q_gp = dp_yield_function(stress_gp, self.TAN_BETA, d_eff_diag)
                        max_f_positive = float(torch.clamp(f_gp, min=0.0).max().item())
                        max_abs_history_f = float(torch.abs(f_gp[PEEQ_data > 1e-12]).max().item()) if bool((PEEQ_data > 1e-12).any()) else 0.0
                        logger.debug(f"[SaveData] Yield surface: max(f+) = {max_f_positive:.6e}, "
                                     f"max|f| at history-plastic points = {max_abs_history_f:.6e}, "
                    ...
                    if hasattr(self, 'TAN_BETA') and hasattr(self, 'cohesion_d'):
                        ...
                        # Use unified verification function with the effective cohesion table
                        max_f_violation, passed = verify_yield_surface(
                            stress=stress_gp,
                            PEEQ=PEEQ_data,
                            tan_beta=self.TAN_BETA,
                            cohesion_d=d_eff_diag,
                            H=0.0,
                            tol=1e-8,
                        ...
                        try:
                            ...
                            f_alt = q_gp - p_gp * tanb - d_eff_diag
        ...
        failfast_msg = getattr(self, '_softening_failfast_pending', None)
        if failfast_msg and self.softening_failfast_enabled:
            self._softening_failfast_pending = None
            raise RuntimeError(failfast_msg)
        self._softening_failfast_pending = None
    ...
    def _effective_cohesion(self, peeq: torch.Tensor) -> torch.Tensor:
        """Return d(PEEQ) from the shared softening table.
        
        CRITICAL per RevisionIdea.md Section 4.3: This is the SINGLE SOURCE OF TRUTH
        for cohesion softening. All yield function evaluations must use this method.
        """
    ...
    def RadialReturn_DP(self, strain, eps_p_old, PEEQ_old, alpha_old):
        ...
        CRITICAL FIX 4: Uses the shared softening table directly for d(PEEQ)
        and its local slope H = dd/dPEEQ.
        """
        # Compute H_d (vectorized) from the shared softening table
        try:
            H_d_vec = self.HardeningModulus(PEEQ_old)
        except Exception:
            # Fallback: if HardeningModulus expects a scalar, create scalar H_d
            H_d_val = self.HardeningModulus(PEEQ_old if isinstance(PEEQ_old, torch.Tensor) else torch.tensor(PEEQ_old))
            H_d_vec = H_d_val if isinstance(H_d_val, torch.Tensor) else torch.full_like(PEEQ_old, H_d_val)

        if isinstance(H_d_vec, torch.Tensor):
            H_d_val = H_d_vec
        else:
            H_d_val = H_d_vec
        ...
        dp_training_mode = bool(getattr(self, '_dp_training_mode', True))
        ...
        if (not dp_training_mode) and hasattr(self, '_debug_yield_check') and self._debug_yield_check:
            with torch.no_grad():
    ...
    def _save_dense_vtk(self, mesh_cache, U_tensor: torch.Tensor, step: int):
        ...
        # Add nodal displacement.  Provide Abaqus-style vector "U" plus scalar
        # components so ParaView comparisons do not accidentally use Ux as the
        # active scalar instead of displacement magnitude.
        U_report_dense = map_sign(U_tensor, 'strain')
        U_np = U_report_dense.detach().cpu().numpy()
        grid.point_data['U'] = U_np
        grid.point_data['U_raw'] = U_tensor.detach().cpu().numpy()
        try:
            grid.set_active_vectors('U')
        except Exception:
            pass
        ...
        try:
            ...
            if Ele_info_dense is not None:
                ...
                old_dp_training_mode = getattr(self, '_dp_training_mode', True)
                old_dense_vtk_flag = getattr(DruckerPragerPlasticity, '_in_dense_vtk', False)
                self._dp_training_mode = False
                DruckerPragerPlasticity._in_dense_vtk = True
                try:
                    ...
                    self._dp_training_mode = old_dp_training_mode
                    DruckerPragerPlasticity._in_dense_vtk = old_dense_vtk_flag

                strain_avg, stress_avg, eps_p_gp, PEEQ_gp, _, stress_actual = outputs
                ...
                # Compute f_plus = max(0, f) for yield surface violation diagnostic (f⁺_max)
                d_eff = self._effective_cohesion(PEEQ_gp)
                f_gp, _, _ = dp_yield_function(stress_actual, self.TAN_BETA, d_eff)
                f_plus_gp = torch.clamp(f_gp, min=0.0)
                f_plus_avg = f_plus_gp.mean(dim=0)
                f_plus_max_step = f_plus_gp.max().item()
                if not hasattr(self, '_f_plus_max_global'):
                    self._f_plus_max_global = 0.0
                self._f_plus_max_global = max(self._f_plus_max_global, f_plus_max_step)
                logger.info(f"Step {step} f⁺_max = {f_plus_max_step:.6e} (global max = {self._f_plus_max_global:.6e})")
                ...
                f_plus_np = f_plus_avg.detach().cpu().numpy()
                vtk_file_dense.write('f_plus', f_plus_np)  # R3-1: yield surface violation diagnostic
