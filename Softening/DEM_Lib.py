...
def _read_seed_from_env(default_seed=2022):
    raw_seed = os.environ.get('DP_SEED', '').strip()
    if not raw_seed:
        return default_seed
    try:
        return int(raw_seed)
    except ValueError:
        logger.warning(f"Invalid DP_SEED='{raw_seed}', falling back to {default_seed}")
        return default_seed


def configure_global_seed():
    seed = _read_seed_from_env(2022)
    random.seed(seed)
    np.random.seed(seed)
...
GLOBAL_SEED = configure_global_seed()
...
_DP_SOFTENING_PEEQ_BREAKS = torch.tensor([0.0, 0.005, 0.02, 0.05, 0.08], dtype=torch.float64)
# Converted from AbaqusVerify/main.inp:
# *DRUCKER PRAGER HARDENING, TYPE=COMPRESSION
# σc = [24.35, 20.0, 16.0, 12.0, 12.0] at the breakpoints above.
# For f=q-p*tan(beta)-d and beta=43.3 deg, d = σc * (1 - tan(beta)/3).
_DP_SOFTENING_COHESION = torch.tensor(
    [16.70124026555583, 13.71765114214031, 10.974120913712248, 8.230590685284186, 8.230590685284186],
    dtype=torch.float64
)


def _dp_softening_lookup(peeq):
    if isinstance(peeq, torch.Tensor):
        peeq_tensor = peeq.to(dtype=torch.float64)
    else:
        peeq_tensor = torch.as_tensor(peeq, dtype=torch.float64, device=dev)

    breakpoints = _DP_SOFTENING_PEEQ_BREAKS.to(device=peeq_tensor.device, dtype=peeq_tensor.dtype)
    values = _DP_SOFTENING_COHESION.to(device=peeq_tensor.device, dtype=peeq_tensor.dtype)
    x = torch.clamp(peeq_tensor, min=breakpoints[0], max=breakpoints[-1])
    segment_idx = torch.searchsorted(breakpoints[1:-1], x, right=True)
    left_bp = breakpoints[segment_idx]
    right_bp = breakpoints[segment_idx + 1]
    left_val = values[segment_idx]
    right_val = values[segment_idx + 1]
    return x, left_bp, right_bp, left_val, right_val


def dp_softening_cohesion(peeq):
    """Piecewise-linear softening table for d(PEEQ)."""
    x, left_bp, right_bp, left_val, right_val = _dp_softening_lookup(peeq)
    span = right_bp - left_bp
    weight = torch.where(span.abs() > 0.0, (x - left_bp) / span, torch.zeros_like(x))
    return left_val + weight * (right_val - left_val)


def dp_softening_hardening_modulus(peeq):
    """Local slope dd/dPEEQ from the shared softening table."""
    _, left_bp, right_bp, left_val, right_val = _dp_softening_lookup(peeq)
    span = right_bp - left_bp
    return torch.where(span.abs() > 0.0, (right_val - left_val) / span, torch.zeros_like(span))


def dp_softening_dissipation(peeq):
    """Positive plastic dissipation integral int_0^PEEQ d(kappa) dkappa."""
    if isinstance(peeq, torch.Tensor):
        peeq_tensor = peeq.to(dtype=torch.float64)
    else:
        peeq_tensor = torch.as_tensor(peeq, dtype=torch.float64, device=dev)
    ...
    breakpoints = _DP_SOFTENING_PEEQ_BREAKS.to(device=peeq_tensor.device, dtype=peeq_tensor.dtype)
    values = _DP_SOFTENING_COHESION.to(device=peeq_tensor.device, dtype=peeq_tensor.dtype)
    x, left_bp, right_bp, left_val, right_val = _dp_softening_lookup(peeq_tensor)
    span = right_bp - left_bp
    slope = torch.where(span.abs() > 0.0, (right_val - left_val) / span, torch.zeros_like(span))

    segment_width = breakpoints[1:] - breakpoints[:-1]
    segment_area = 0.5 * (values[:-1] + values[1:]) * segment_width
    cumulative_area = torch.cat([
        torch.zeros(1, dtype=peeq_tensor.dtype, device=peeq_tensor.device),
        torch.cumsum(segment_area, dim=0)
    ])

    segment_idx = torch.searchsorted(breakpoints[1:-1], x, right=True)
    dx = x - left_bp
    integral = cumulative_area[segment_idx] + left_val * dx + 0.5 * slope * dx * dx

    # Abaqus-style tabular softening holds the last value after the final point.
    tail = torch.clamp(peeq_tensor - breakpoints[-1], min=0.0) * values[-1]
...
def verify_yield_surface(stress, PEEQ, tan_beta, cohesion_d, H, tol=1e-8, context="", only_plastic: bool = False):
    ...
    """
    # Effective cohesion must use the same tabular softening law as the return
    # mapping.  The old linear d+H*PEEQ shortcut is only valid inside one table
    # segment and can produce false yield diagnostics after a breakpoint.
    try:
        d_eff = dp_softening_cohesion(PEEQ)
    except Exception:
...
def setup_domain(file, BoundingBox):
        ...
        with open(input_path, 'r', encoding='utf-8', errors='ignore') as fp:
            for line in fp:
                ...
                if readNode:
                    ...
                    coords = (coords[:3] + [0.0] * 3)[:3]  # ensure exactly 3 coords (x, y, z)
                    ...
                    # Abaqus allows element connectivity to continue on the next line
                    # when a line ends with a trailing comma. Buffer until the logical
                    # element card is complete before parsing node ids.
                    element_line_buffer += (" " if element_line_buffer else "") + stripped
                    if stripped.endswith(','):
                        continue

                    raw_parts = [p.strip() for p in element_line_buffer.replace('\n', ' ').split(',') if p]
                    element_line_buffer = ""
                    if not raw_parts:
                        continue
                    ...
                    conn_len = len(conn)
                    if conn_len not in (4, 8):
                        unsupported_elements += 1
                        if len(unsupported_element_examples) < 5:
                            unsupported_element_examples.append(
                                f"elem {elem_label if elem_label is not None else '?'}: {conn_len} nodes"
                            )
                        continue

                    if expected_conn_len is None:
                        expected_conn_len = conn_len
                    elif conn_len != expected_conn_len:
                        raise ValueError(
                            f"Mixed element types detected in {input_path}: "
                            f"expected {expected_conn_len}-node elements, got {conn_len}-node element "
                            f"at elem {elem_label if elem_label is not None else '?'}."
                        )
    ...
    # Read Abaqus node sets in a second pass. Boundary conditions and reaction
    # extraction must use the same node sets as the Abaqus reference model;
    # coordinate bands can accidentally include external reference nodes.
    try:
        active_nset = None
        ...
        with open(input_path, 'r', encoding='utf-8', errors='ignore') as fp:
            for line in fp:
                ...
                if stripped.startswith('*'):
                    active_nset = None
                    active_generate = False
                    card_lower = stripped.lower()
                    if card_lower.startswith('*nset'):
                        nset_name = None
                        active_generate = False
                        for option in stripped.split(',')[1:]:
                            opt = option.strip()
                            opt_lower = opt.lower()
                            if opt_lower.startswith('nset='):
                                nset_name = opt.split('=', 1)[1].strip().strip('"').upper()
                            elif opt_lower == 'generate':
                                active_generate = True
                        if nset_name:
                            active_nset = nset_name
                            node_sets_raw.setdefault(active_nset, [])
                    continue

                if active_nset is None:
                    continue

                try:
                    values = [int(p.strip()) for p in stripped.split(',') if p.strip()]
                except ValueError:
                    continue

                if active_generate:
                    for i in range(0, len(values) - 2, 3):
                        start, stop, inc = values[i], values[i + 1], values[i + 2]
                        if inc == 0:
                            continue
                        node_sets_raw[active_nset].extend(range(start, stop + (1 if inc > 0 else -1), inc))
                else:
                    node_sets_raw[active_nset].extend(values)
    except FileNotFoundError:
        logger.warning(f"Could not reopen input file to read node sets: {input_path}")
    ...
    if element_line_buffer.strip():
        raise ValueError(
            f"Unterminated element line encountered in {input_path}: "
            f"'{element_line_buffer.strip()[:120]}'"
        )

    if unsupported_elements:
        detail_msg = ""
        if unsupported_element_examples:
            detail_msg = " Examples: " + "; ".join(unsupported_element_examples)
        raise ValueError(
            f"Unsupported element connectivity encountered in {input_path}: "
            f"{unsupported_elements} element(s) were not 4-node or 8-node.{detail_msg}"
        )
    ...
    node_sets = {}
    for set_name, labels in node_sets_raw.items():
        mapped = []
        seen = set()
        for node_label in labels:
            node_idx = node_id_map.get(node_label)
            if node_idx is None or node_idx in seen:
                continue
            mapped.append(node_idx)
            seen.add(node_idx)
        if mapped:
            node_sets[set_name] = torch.as_tensor(mapped, dtype=torch.long, device=dev)
    ...
    if node_sets:
        key_sets = ['TOP', 'BOTTOM', 'LEFT', 'FRONT']
        counts = [f"{name}={int(node_sets[name].numel())}" for name in key_sets if name in node_sets]
        if counts:
            logger.info(f"Loaded Abaqus node sets: {', '.join(counts)}")
...
def load_top_surface_history(csv_path):
    ...
    if "top_disp_uni" not in names and "top_disp" in names:
        data = rfn.append_fields(
            data,
            "top_disp_uni",
            np.asarray(data["top_disp"], dtype=float),
            usemask=False,
        )
        names = set(data.dtype.names or [])

    if "top_s33_uni" not in names:
        if "s33_volmean" in names:
            source_col = "s33_volmean"
        elif "s33_from_reaction" in names:
            source_col = "s33_from_reaction"
        elif "s22_volmean" in names:
            source_col = "s22_volmean"
        elif "s22_from_reaction" in names:
            source_col = "s22_from_reaction"
        elif "top_reaction_force" in names:
            source_col = "top_reaction_force"
        else:
            source_col = None
        if source_col is not None:
            data = rfn.append_fields(
                data,
                "top_s33_uni",
                np.asarray(data[source_col], dtype=float),
                usemask=False,
            )
            names = set(data.dtype.names or [])
...
def query_top_surface_csv(
    ...
    if column not in data.dtype.names:
        aliases = {
            "top_s33_uni": ("s33_volmean", "s33_from_reaction", "s22_volmean", "s22_from_reaction", "top_reaction_force"),
            "top_disp_uni": ("top_disp",),
        }
        for alt in aliases.get(column, ()):
            if alt in data.dtype.names:
                column = alt
                break
        else:
