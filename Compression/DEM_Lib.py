...
if torch.cuda.is_available():
    ...
    device_string = 'cuda'
    torch.set_default_dtype(torch.float64)  # Changed to float64 for numerical stability
...
def force_consistency_check(face_force, assembled_force, tol=0.02):
    """Compare surface (face) and assembled forces and return (within_tol, rel_err, diff).
    
    Per RevisionIdea.md Section 3: Default tolerance is 1-2% (0.01-0.02) for
    discretization-level agreement, not 1e-6 which is too strict.
    """
...
def dp_yield_function(stress: torch.Tensor, tan_beta, cohesion_term):
    """Evaluate f = q - p*tan(beta) - d using consistent invariants."""
...
def verify_yield_surface(stress, PEEQ, tan_beta, cohesion_d, H, tol=1e-8, context=""):
    """
...
def verify_force_consistency(F_energy, F_face, F_asm, tol=0.02, context=""):
    """
    Verify consistency between energy-derived, face-integrated, and assembled forces.
    
    Per RevisionIdea.md Section 3: Check F_energy ≈ F_face ≈ F_asm within 1-2% tolerance.
    ...
    Args:
        ...
        tol: Relative tolerance (default 0.02 = 2%)
        context: String describing where this check is called from
...
def setup_domain(file, BoundingBox):
    ...
    try:
        with open(f"{file}.inp", 'r', encoding='utf-8', errors='ignore') as fp:
            for line in fp:
                if '*Node' in line:
                    readNode = True
                    continue
                ...
                if readNode:
                    ...
                    if node_id != 300000:
                        ...
                        logger.info(f"Ignoring node 300000 with coordinates: {coords}")

                if readEle:
                    parts = [p.strip() for p in line.strip().split(',')[1:] if p]
                    if not parts:
                        continue
                    try:
                        conn = [int(t) - 1 for t in parts]
                    except ValueError:
                        continue
                    if len(conn) == 8:
        ...
        logger.error(f"Input file not found: {file}.inp")
        return None
    ...
    if CellType == 12:
        logger.info('Found Hexahedral mesh for uniaxial compression!')
    elif CellType == 10:
        logger.info('Found Tetrahedral mesh for uniaxial compression!')
...
class S_Net(torch.nn.Module):
    ...
    def __init__(self, D_in, H, D_out , act_fn):
        ...
        try:
            ...
            # When using positional encoding, the input size to linear1 changes
            # PositionalEncoding with m=10 creates 2*m*D_in features = 2*10*3 = 60
            encoded_size = 2 * 10 * D_in
            self.linear1 = torch.nn.Linear(encoded_size, H)
        except ImportError:
            logger.warning("rff module not available, skipping positional encoding")
    ...
    def forward(self, x ):
        ...
        if self.use_encoding:
            y = self.encoding(x)
            y = activation_fn(self.linear1(y))
        else:
    ...
    if len(dPEEQ) > 0:  # If at least one point is yielding
        ...
        if not KINEMATIC:
            ...
            for itr in range( Num_Newton_itr ):
                ...
                c_pl = (trial_s_eff[yield_flag] - flow_stress[yield_flag] - 3. * mu * dPEEQ) / (3. * mu + H_curr + eps_global)
                dPEEQ = dPEEQ + c_pl
                
                # Check for convergence or NaN
                if torch.max(torch.abs(c_pl)) < 1e-10:  # Tightened from 1e-9 to 1e-10
                    break
                if torch.isnan(c_pl).any():
                    logger.warning("NaN detected in RadialReturn c_pl. Breaking loop.")
                    break
