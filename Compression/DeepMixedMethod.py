...
class DruckerPragerPlasticity(torch.autograd.Function):
    """
    Drucker-Prager plasticity with a configurable flow rule.
    ...
    4. PEEQ update: ΔPEEQ = Δγ as the scalar plastic multiplier measure
    ...
    def forward(ctx, strain, eps_p_old, PEEQ_old, D_tensor, G, K, TAN_BETA, TAN_PSI, cohesion_d, H):
        ...
        with torch.no_grad():
            ...
            if plastic_mask.any():
                ...
                # Drucker-Prager return mapping with configurable dilation angle.
                # Yield function: f = q - p*tan(β) - d = 0
                # Smooth-cone update denominator contains tan(β)*tan(ψ).
                ...
                # Track equivalent plastic strain using the plastic multiplier.
...
class DeepMixedMethod:
    ...
    Differentiable Drucker-Prager return mapping for neural energy training.

    CORE PROBLEM IDENTIFIED: stable differentiable plasticity in neural training
    - Non-associated flow leads to non-symmetric algorithmic tangents
    - Mixed forward/backward definitions can cause premature yielding and instability

    ROOT CAUSE SOLUTION: Drucker-Prager return mapping with corrected mathematics
    - Forward: configurable flow rule and correct NR residual/update
    - Backward: tangent mode controlled by DP_TANGENT_MODE for ablation
    ...
    def __init__(self, model):
        ...
        # Estimate yield displacement for dynamic penalty activation.
        ...
        if self.EXAMPLE in [4,5]:
            ...
            if hasattr(self, 'cohesion_d'):
                ...
                logger.info(f"Dilation Angle: {self.DILATION_ANGLE}° (NON-ASSOCIATED FLOW)")
                logger.info(f"TAN_BETA: {self.TAN_BETA.item():.6f}")
                logger.info(f"TAN_PSI: {self.TAN_PSI.item():.6f}")
                logger.info(f"*** FLOW RULE: Non-associated Drucker-Prager plasticity ***")
                logger.info(f"*** TANGENT MODE: {DP_TANGENT_MODE} ***")
                ...
                logger.info(f"  *** NUMERICAL NOTE: Tangent mode controlled by DP_TANGENT_MODE ***")
    ...
    # Prepare checkpoint directory for robust weight transfer between steps
        ...
        for step in range(1, self.step_max+1):
            ...
            # Re-enable autograd for the authoritative training-mesh evaluation so the
            # constitutive update follows the same path as optimization, not the inference-only guardrail.
    ...
    def getUP(self, nodes, disp_target_val):
        ...
        N = self.S_Net(x_in_net).to(dtype=self.physics_dtype) # Shape [nN, 3]
    ...
    def reaction_via_energy(self, nodesEn, eps_p, PEEQ, alpha, applied_disp):
        ...
        # applied_disp is negative for compression. With this parameterization,
        # dΠ/da has the same raw sign as the face/assembled top reaction used below.
    ...
    def SaveData( self , domain , U , ip_out , final_state, LBFGS_loss , step , ref_file ):
        ...
        try:
            ...
            z_top = torch.max(nodesEn[:, 2])
            top_tol_val = max(1e-6, 1e-2 * max(abs(float(z_top.item())), 1.0))
            top_mask = nodesEn[:, 2] >= (z_top - top_tol_val)
            ...
            try:
                ...
                on_top_all = node_z_all >= (z_top - top_tol_val)
